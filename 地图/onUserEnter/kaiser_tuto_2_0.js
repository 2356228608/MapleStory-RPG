...
ms.updateInfoQuest(58977, "lastDay=19/09/22/17/23");
...
ms.updateInfoQuest(18192, "count=0;val2=0;tDate=19/09/22/17/23;val=0");
ms.updateInfoQuest(500208, "count=0;val2=0;tDate=19/09/22/17/23;val=0");
ms.updateInfoQuest(500209, "count=0;val2=0;tDate=19/09/22/17/23;val=0");
...
ms.npc_LeaveField("oid=9813952");

ms.npc_ChangeController(3000107, "oid=9814344", -2000, 20, 0);
ms.npc_SetSpecialAction("oid=9814344", "summon", 0, 0);
ms.onScriptMessage(3, 0, 1, 3000107,  "没想到竟然连这里都出现了幽灵……", 256, 0, 17, 0, 0, 0); // [类型] 普通对话
} else if (status === i++) {
ms.curNodeEventEnd(true);
ms.onScriptMessage(3, 0, 1, 3000107,  "看来情况比想象的更严重！", 257, 0, 1, 0, 0, 0); // [类型] 普通对话
} else if (status === i++) {
ms.inGameDirectionEvent_MoveAction(1);
ms.inGameDirectionEvent_AskAnswerTime(30);
} else if (status === i++) {
ms.inGameDirectionEvent_MoveAction(0);
ms.onScriptMessage(3, 0, 1, 3000107,  "我有不好的预感。请你赶紧返回，立即建立防御膜，做好万全准备。", 256, 0, 3, 0, 0, 0); // [类型] 普通对话
} else if (status === i++) {
ms.onScriptMessage(3, 0, 1, 3000107,  "越是这种时候，我才更应该跟你一起去。虽然你是狂龙战士，可是单枪匹马对付他们……", 257, 0, 1, 0, 0, 0); // [类型] 普通对话
} else if (status === i++) {
ms.onScriptMessage(3, 0, 1, 3000107,  "卡塔利安，你是诺巴骑士。你难道忘了，比性命更宝贵的首要任务正是保卫诺巴族的安全？而且我是狂龙战士，从来只有我操心别人的，才没有弱到需要别人操心呢。", 257, 0, 3, 0, 0, 0); // [类型] 普通对话
} else if (status === i++) {
ms.onScriptMessage(3, 0, 1, 3000107,  "既然你这么坚持……那么只能祝你好运了。", 257, 0, 1, 0, 0, 0); // [类型] 普通对话
} else if (status === i++) {
ms.inGameDirectionEvent_AskAnswerTime(1000);
} else if (status === i++) {
ms.setInGameDirectionMode(false, true, false);
ms.npc_LeaveField("oid=9814344");
ms.dispose();
ms.warp(940001100, 0);
    }
}