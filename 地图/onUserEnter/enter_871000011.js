...
function action(mode, type, selection) {
    ...
    var i = -1;
        if (status <= i++) {
			...
			ms.sendNextNoESC_Bottom("#b…?");
		} else if (status === i++) {
			ms.sendNextNoESC_Bottom("#b难道是我走错了？");
		} else if (status === i++) {
			...
			ms.sendNextNoESC_Bottom("#b……我的50万金币！！！");
		} else if (status === i++) {
			...
			ms.sendNextNoESC_Bottom("#face3#吓死我了，你是谁啊？");
		} else if (status === i++) {
			ms.sendNextNoESC_Bottom("#b嗯……我是刚搬过来的人。",9400921);
		} else if (status === i++) {
			ms.sendNextNoESC_Bottom("#face3#搬家？嗯……我还以为这房屋没有主人呢，看来不是啊。",9400921);
		} else if (status === i++) {
			ms.sendNextNoESC_Bottom("#face1#我是住在旁边的人，叫埃尔宾。相见就是缘分，既然我们成了邻居，那就好好相处吧。很高兴见到你！ ",9400921);
		} else if (status === i++) {
			ms.sendNextNoESC_Bottom("#b我也很高兴……虽然不知道这个房屋能不能住人，但是以后也请多多指教了。");
		} else if (status === i++) {
			ms.sendNextNoESC_Bottom("#face2#嗯~~看起来很小巧，你一个人生活的话应该够了啊？只要整理一下就好了！我会帮忙的，你不用担心。 ",9400921);
		} else if (status === i++) {
			ms.sendNextNoESC_Bottom("#face1#首先把这个破椅子拿开吧。 ",9400921);
		} else if (status === i++) {
			ms.sendNextNoESC_Bottom("#face1#这个我来收拾，你把床搬到日照充足的地方去吧？");
		} else if (status === i++) {
            ms.fieldEffect_ProcessOnOffLayer("0", "Effect/DirectionWz2.img/effect/myHomeTuto/2",0, 900, 0, -80, 12, 4, 0);//添加图层
            ms.inGameDirectionEvent_AskAnswerTime(300);
        } else if (status === i++) {
			ms.sendNextNoESC_Bottom("#face1#点击我的小屋菜单的#i3801392##t3801392#就能进入可以搬动家具的放置模式。",9400921);
		}  else if (status === i++) {
			ms.sendNextNoESC_Bottom("#face1#在放置模式状态中，点击床后再点击你想要放置的位置即可！",9400921);
		} else if (status === i++) {
			ms.fieldEffect_ProcessOnOffLayer("0", "", 2, 1000, 0, 0, 0, 0, 0);//移除图层
            ms.inGameDirectionEvent_AskAnswerTime(2000);
        } else if (status === i++) {
			ms.sendNextNoESC_Bottom("#face2#你要不要试试？将床放到你想要放的地方后再和我对话吧。",9400921);
        } else if (status === i++) {
			ms.curNodeEventEnd(true);//角色站立方向信息
