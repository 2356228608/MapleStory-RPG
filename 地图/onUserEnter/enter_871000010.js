...
function action(mode, type, selection) {
    ...
    var i = -1;
        if (status <= i++) {
			...
			ms.sendNextS_Bottom("#face3#哎呀……你是看了宣传单后来的吗？真的？",9400920);
		} else if (status === i++) {
			ms.sendNextS_Bottom("#face2#哎呀呀！别站着了，快点坐在这里吧！ ",9400920);
		} else if (status === i++) {
			ms.sendNextS_Bottom("#face2#虽然你已经看过宣传单了，但你应该知道这不是什么华丽的大宅子哦。它非常小巧，有点朴素但是非常温馨~，真的是很舒适的家哦。",9400920);
		} else if (status === i++) {
			ms.sendNextS_Bottom("#face1#请想象一下在结束了疲惫的一天后，躺在床上休息的样子。远处的鸟叫声，从窗子外面吹来的微风。可以在其中舒舒服服地休息！这不就是幸福吗！",9400920);
		} else if (status === i++) {
			ms.sendNextS_Bottom("#face1#家是回忆的场所，为了能让新的主人用回忆填满这个地方，家里已经留下了一些~空闲的地方。这样的留白总是让人激动呢。 ",9400920);
		} else if (status === i++) {
			ms.askMenu_Bottom("#face2#这样的房屋只要50万金币！怎么样？现在就马上签合同吧？\r\n#b#L0#我要签合同。#l\r\n#L1#给我一点时间考虑一下。#l\r\n#L2#这个价格是不是太便宜了。#l\r\n#L3#你这话说的有点奇怪啊。#l",9400920);
		}  else if (status === i++) {
			switch(selection){
				case 0://签合同
					ms.sendNextS_Bottom("#face3#嗯~现在想要签合同的人可都排起了长队呢，我可等不了太久。你应该知道这是个千载难逢的机会吧？",9400920);
					break;
				case 1:
					ms.sendNextS_Bottom("#face3#嗯~现在想要签合同的人可都排起了长队呢，我可等不了太久。你应该知道这是个千载难逢的机会吧？",9400920);
					status-=2;
					break;
				case 2:
					ms.sendNextS_Bottom("#face3#啊，啊？你是觉得太便宜了，所以在怀疑吗？…… \r\n虽然我觉得卖房不应该说这些……但是既然你在怀疑，那就没办法了。",9400920);
					status-=2;
					break;
				case 3:
					ms.sendNextS_Bottom("#face3#我的父母都是冒险家。虽然不是像勇士大人这么有名的冒险家，但也是为了冒险岛世界的和平而奋不顾身的人。他们把我和哥哥丢在这里……（抽泣）,但是比起怨恨，我心中更多的是担心和思念。我们已经和他们失去联系很久了……（抽泣）所以我打算卖掉房子去寻找父母。你听说过[抛售]吗？抛！售！",9400920);
					status-=2;
					break;
			}
		} else if (status === i++) {
			ms.sendNextS_Bottom("#face3#哎呀，你这人怎么这么多疑！难道你担心我是把这些话写好之后背下来的吗？！ ",9400920);
		} else if (status === i++) {
			ms.sendNextS_Bottom("#face2#好的！请在这个合同上签字吧！ ",9400920);
		} else if (status === i++) {
			ms.fieldEffect_InsertCanvas(1, 150, 0, 0, 0, 300,0);//创建一个遮布 0x10
			...
			ms.sendNextS_Bottom("#face1#啊，我本来打算给你能够马上前往家中的#i2630583:##t2630583#，但是背包里似乎没有空间了呢！",9400920);
		} else if (status === i++) {
			ms.sendNextS_Bottom("#face2#请不要担心，即使没有#t2630583#，还有很多可以前往家中的方法！",9400920);
		} else if (status === i++) {
            ms.fieldEffect_ProcessOnOffLayer("0", "Effect/DirectionWz2.img/effect/myHomeTuto/0",0, 900, 0, -80, 12, 4, 0);//添加图层
            ms.inGameDirectionEvent_AskAnswerTime(300);
        } else if (status === i++) {
            ms.sendNextS_Bottom("#face1#第一，点击角色快捷栏的[前往我的小屋]按键后移动。",9400920);
        } else if (status === i++) {
            ms.sendNextS_Bottom("#face1#第二，在我的角色信息栏中点击[前往我的小屋]按键后移动。",9400920);
        } else if (status === i++) {
			ms.fieldEffect_ProcessOnOffLayer("0", "", 2, 300, 0, 0, 0, 0, 0);//移动图层
            ms.inGameDirectionEvent_AskAnswerTime(2000);
        } else if (status === i++) {
            ms.sendNextS_Bottom("#face1#我接下来还有事情，就不能和你一起去了……你应该能找到吧？",9400920);
        } else if (status === i++) {
            ms.sendNextS_Bottom("#face3#那就请好好享受冒险岛生活吧！ 下次再见……#fs8#(别再见)……#fs12#了！",9400920);
        }  else if (status === i++) {
			ms.curNodeEventEnd(true);//角色站立方向信息
