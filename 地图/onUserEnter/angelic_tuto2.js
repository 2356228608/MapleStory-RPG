...
function action(mode, type, selection) {
    ...
    if (status <= i++) {
        ...
        ms.npc_ChangeController(3000140, -1400, 0, 1);//51 5A 00 01
        ms.npc_SetSpecialAction(3000140, "summon");
        ms.npc_ChangeController(3000104, -1650, 0, 0);//52 5A 00 01
        ms.npc_SetSpecialAction(3000104, "summon");
        ms.sendNextNoESC("嗯，天气真好！", 3000101);
    } else if (status === i++) {
        ms.sendNextNoESC("你也太清闲了。我和#h0#已经成为骑士团团员了，你打算什么时候加入骑士团？", 3000104);
    } else if (status === i++) {
        ms.sendNextNoESC("嘿嘿，我嘛，没有魔力，就不提这些咯。说真的，也许将来我也会有魔力吧？", 3000104);
    } else if (status === i++) {
        ...
        ms.npc_SetForceMove(3000140, 1, 400, 100);
