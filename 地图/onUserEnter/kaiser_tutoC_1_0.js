...
function action(mode, type, selection) {
    ...
    if (status <= i++) {
        ...
        ms.npc_ChangeController(3000103, -300, 220, 0);
        ...
        ms.inGameDirectionEvent_Effect("Effect/Direction9.img/effect/story/BalloonMsg0/0", 1200, 0, -100, 1, 0, 1, 3000103, 0, 0);
        ...
        ms.sendNextNoESC("你怎么那么迷信？这可是千载难逢的好机会！现在我们不能回头了！", 3000111);
    } else if (status === i++) {
        ms.sendNextNoESC("你要拿走圣物吗？", 3000115);
    } else if (status === i++) {
        ms.sendNextNoESC("必须阻止他们！！", 3000103);
    } else if (status === i++) {
        ms.npc_SetForceMove(3000103, 1, 300, 100);
        ms.inGameDirectionEvent_AskAnswerTime(300);
    } else if (status === i++) {
        ms.inGameDirectionEvent_MoveAction(2);
        ms.inGameDirectionEvent_Effect("Effect/Direction9.img/effect/story/BalloonMsg1/1", 1200, 0, -140, 1, 0, 1, 3000110, 0, 0);
