...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        qm.askAcceptDecline("修炼进展得如何？哟，等级升得这么高了？难怪人们都说济州岛是养马的天堂，金银岛是升级的天堂……对了，现在还不是说闲话的时候。能否麻烦你回岛上来一趟？");
    } else if (status == 1) {
...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.askYesNo("说声不好意思就能算了？！几百年来就我一个人孤苦伶仃地，有多寂寞你知道吗？不管怎样，你快点给我想起来！");
    } else if (status == 12) {
        ...
        qm.askYesNo("Would you like to skip the video clip?  Even if you skip the scene, game play will not be affected.");
    } else if (status == 14) {
        qm.dispose();
    }
}