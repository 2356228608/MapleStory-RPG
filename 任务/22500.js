...
function start(mode, type, selection) {
    ...
    if (status == 1)
        qm.sendNextPrevS("#bWhoooooa, it talks!");
    if (status == 2)
    ...
    if (status == 3)
        qm.sendNextPrevS("#bEh? What do you mean? We'll be seeing a lot of each other? What pact?");
    if (status == 4)
    ...
    if (status == 5)
        qm.sendNextPrevS("#bWhaaat? A Dragon? You're a Dragon?! I don't get it... Why am I your master? What are you talking about?");
    if (status == 6)
    ...
    if (status == 7)
        qm.sendNextPrevS("#bWait, wait, wait. Let me get this straight. You're saying I have no choice but to help you?");
    if (status == 8)
        qm.sendNextPrev("Yuuup! Heeeey...! What's with the face? You...don't want to be my master?");
    if (status == 9)
        qm.sendNextPrevS("#bNo... It's not that... I just don't know if I'm ready for a pet.");
    if (status == 10)
        qm.sendNextPrev("A p-p-pet?! Did you just call me a pet?! How dare... Why, I'm a Dragon! The strongest being in the world!");
    if (status == 11)
        qm.sendNextPrevS("#b...#b(You stare at him skeptically. He looks like a lizard. A puny little one, at that.)#k");
    if (status == 12)
        qm.askYesNo("Why are you looking at me like that?! Just watch! See what I can do with my power. Ready?");
    if (status == 13) {
        qm.forceStartQuest();
    ...
    if (status == 15) {
        qm.evanTutorial("UI/tutorial/evan/11/0");
        qm.dispose();
    }
}
