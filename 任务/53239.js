...
function start(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            qm.sendNextSNoESC("Toi can 1 khau sung moi. Toi da manh len rat nhieu.");
        } else if (status == 2) {
            ...
            qm.sendNextSNoESC("(Oh men, lan nay khong biet lai gi nua day. Chep, danh vay)");
        } else if (status == 5) {
            qm.forceStartQuest();
            qm.warp(552000073, 1);
...
function end(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            qm.gainItem(4033252, -30);
            qm.sendNextSNoESC("Ong lam xong sung cho toi roi chu?");
        } else if (status == 2) {
