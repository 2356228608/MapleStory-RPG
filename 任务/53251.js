var status = -1;
function start(mode, type, selection) {
//    if (mode == -1) {
//        qm.dispose();
//    } else {
//        if (mode == 1)
//            status++;
//        else
//            status--;
//        if (status == 0) {
//            qm.sendNext("La cau day ak, co viec gi the",2040050);
//        } else if (status == 1) {
//            qm.sendNextSNoESC("Toi muon biet nguoi dua cho anh buc thu la ai!");
//        } else if (status == 2) {
//            qm.sendNext("Anh ay la nguoi mac ao den, co 2 mat, 1 mui, 2 tai...blah blah!",2040050);
//        } else if (status == 3) {
//            qm.sendNextSNoESC("Dung dua toi nua! Noi cho biet anh ay la ai");
//        } else if (status == 4) {
//            qm.sendNext("Anh that chang biet dua! Tai sao toi phai giup anh nhi?her",2040050);
//        } else if (status == 5) {
//            qm.sendNextSNoESC("Toi xin loi. Thuc su dieu do rat quan trong doi voi toi luc nay.");
//        } else if (status == 6) {
//            qm.sendNext("Cau muon gap anh ay chu?");//,2040050);
//        } else if (status == 7) {
//            //qm.forceStartQuest();
//            qm.warp(552000074);
//            qm.dispose();
//        }
//    }

}
function end(mode, type, selection) {
    qm.dispose();
}
