...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.askYesNo("怎么样？你想一起踏上飞侠之路吗？如果你决定选择飞侠之路，我就使用转职官的特权，邀请你到#b废都的废都酒吧#k去……那是个隐秘的地方，你应该感到荣幸。#r但是如果更喜欢其他职业的话，你可以拒绝。我会为你推荐飞侠之外的其他道路#k。");
    } else if (status == 4) {
        qm.warp(103000003);
        qm.forceStartQuest();
        qm.dispose();
    } else if (status == 5) {
        qm.askMenu("你不喜欢飞侠之路吗？不喜欢的话，我也不想勉强。那你想选择什么职业呢？\r\n#b#L1#战士#l \r\n#b#L2#魔法师#l \r\n#b#L3#弓箭手#l \r\n#b#L5#海盗#l");
    } else if (status == 6) {
        if (selection == 1) {
...
function end(mode, type, selection) {
    status++;
    if (status == 0) {
        qm.askYesNo("很高兴能在这里见到你……我来把你变成飞侠吧。做好心理准备了吗？临阵退缩的人，不配成为飞侠。");
    } else if (status == 1) {
        if (mode == 1) {
