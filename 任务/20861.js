...
function end(mode, type, selection) {
	...
	if (status == 0) {
		qm.askYesNo("你做好了决定吗？一旦决定了就无法再更改，因此在确定职业道路之前要仔细考虑。 你确定要成为一名魂骑士吗？");
	} else if (status == 1) {
		qm.sendNext("我重塑了你的身体，使它能完美匹配魂骑士的道路。如果你希望变得更加强大, 在状态窗口里 (S) 提升能力点。如果你不知道如何搭配，就点击 #b自动#k。");
		if (qm.getJob() != 1100) {
		...
		qm.sendNextPrev("与你作为新手时不同，现在你成为了魂骑士，当你的生命值耗尽后，你将丢失一部分经验值，明白了吗？");
	} else if (status == 5) {
