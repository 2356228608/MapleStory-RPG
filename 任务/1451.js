...
function start(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            qm.askYesNo("现在，这对你的考验时间。#r天鹰#k和#r火焰龙#k 有权承认真正的英雄。你的任务是击败他们，获得#b#t4031343##k和#b#t4031344##k。");
        } else if (status == 4) {
            qm.forceStartQuest();
...
function end(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            qm.askYesNo("干得漂亮, 你确定要进行4转了吗?");
        } else if (status == 1) {
            if (qm.haveItem(4031343, 1) || qm.haveItem(4031344, 1)) {
                qm.removeAll(4031343);//英雄五角星
                qm.removeAll(4031344);//英雄之星
