...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.sendNextPrevS("#b（这药水怎么喝？……不记得了……）#k");
    } else if (status == 6) {
        qm.hireTutorMsg(0xE);
        qm.dispose();
    }
...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.sendNext("你升级了吗？不知道你有没有得到技能点数？在冒险岛世界，每升1级就能获得技能点数3。按#bK键#k，打开技能栏就可确认。");
    } else if (status == 3) {
        qm.sendNextS("#b（对我这么亲切，我却什么都想不起来。我真的是英雄吗？还是先查看一下技能吧……怎么查看技能呀？）#k");
    } else if (status == 4) {
