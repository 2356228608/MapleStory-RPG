...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        qm.sendNextNoESC_New("训练新兵 #h0# 吗? 从训练兵团有接收到物品赠送的事项. 请跟我来领支给用品吧。", 0x21, 1);
    } else if (status == 1) {
...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        qm.sendNextNoESC_New("为了支给用品来的吗? 名称是…嗯…阿! 在这， #h0# 没错吧? 赠送给你立体机动装置吧。", 0x21, 1);
    } else if (status == 1) {
        ...
        qm.sendOk_New("请穿用看看是否合身以及功能是否正常。\r\n以防万一弄不见了还会支配，因此到时来找我。");
        qm.dispose();
    } else {
        qm.dispose();
    }
