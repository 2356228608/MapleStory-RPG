...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        qm.askYesNo("We cannot let those aliens take out precious ore! We need to. I don't know, sabotage them? That sounds right. doesn't it! Go sabotage the heck out of them!");
    } else if (status == 1) {
...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.sendNextSNoESC("The aliens are holding a bunch of people hostage! I'm not sure why yet, but I have a very strong theory that people-eating is involved! They must be interstellar space chefs!");
    } else if (status == 3) {
