...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.askAcceptDecline("来，给你#b三明治，吃完之后到妈妈那里去一趟。#k她好象有话要跟你说。");
    } else if (status == 2) {
        qm.forceStartQuest();
        qm.gainItem(2022620, 1);
        qm.sendNextPrevS("#b(有话要跟我说？先把#t2022620#吃了，然后到屋里去看看吧。)#k");
    } else if (status == 3) {
        qm.evanTutorial("UI/tutorial/evan/3/0");
        qm.dispose();
    }
}
...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.evanTutorial("UI/tutorial/evan/4/0");
        qm.dispose();
    }
}