...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.askAcceptDecline("在找到#o1210100#之前，必须先把坏了的篱笆修好。还好坏得不是太严重，只要有几个#t4032498#应该就能修好了。小不点，要是你能帮我搜集#b3个#t4032498##k就好了……");
    } else if (status == 2) {
        qm.forceStartQuest();
        qm.sendNext("哦，真是谢谢你。#b#t4032498##k可以从周围的#r#o0130100##k身上搜集到。它们虽然不是很强，但不小心的话，可能会遇到危险。你一定要好好使用技能道具。");
    } else if (status == 3) {
        qm.evanTutorial("UI/tutorial/evan/6/0");
        qm.dispose();
    }
}
...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.enableActions();
		qm.evanTutorial("UI/tutorial/evan/7/0");
		qm.dispose();
        qm.sendNext("好了，我用修理篱笆剩下的木板做了一把椅子。虽然不太好看，但却很结实。就给你用吧。");
    } 
}