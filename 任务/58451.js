...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        qm.sendNextS_New("K 城墙崩溃了! 村民陷入危险耶!！", 0x38, 1);
    } else if (status == 1) {
        ...
        qm.askYesNo("我想一下~ 小小的东西就可以了! 去看看杂货商店如何? 应该会有跟这徽章能交换的道具吧? 那我在这里等你把东西找到吧?。");
    } else if (status == 6) {
        qm.forceStartQuest();
        qm.dispose();
    } else {
...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.sendNextS_New("那，就用这徽章交换没有问题吗？", 0x38, 1);
    } else if (status == 1) {
