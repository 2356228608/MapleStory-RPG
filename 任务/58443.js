...
function start(mode, type, selection) {
    if (mode == 1) {
        ...
        if (status == 0) {
            qm.sendOk_New("不是说想帮助饥饿的小孩吗？变心了吗？");
            qm.dispose();
            return;
        }
        status--;
    ...
    if (status == 0) {
        qm.askYesNo("第一次尝试的关系，先选择简单的东西吧…可以从枫之谷世界怪物中取得的马铃薯如何呢？请打倒枫之谷怪物收集50个马铃薯。");
    } else if (status == 1) {
        qm.sendOk_New("希望这事情可以顺利完成.。");
        qm.forceStartQuest();
        qm.dispose();
    } else {
        qm.dispose();
...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.sendNextS_New("来，先吃点这些东西吧。  ", 0x38, 1);
    } else if (status == 1) {
