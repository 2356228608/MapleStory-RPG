...
function action(m, t, s) {
    if (m < 1)
        ...
        if (status == 0) {
            ...
            if (s < 69) {
                e += "/chooseone\r\n\r\n#b#e";
                for (i = 0; i < prizes[s].length; i++)
                    e += "#L" + i + "#Tier " + i + " : #t" + prizes[s][i][0] + "# #v" + prizes[s][i][0] + "##l\r\n";
            } else {
                if (s == 69)
                    e += "This is the advanced requirement tree, it is faster to browse. If you do not understand it, please choose the simplified requirement tree instead, you may find it more suited to you.\r\n\r\n" + parseTree(true);
                else
...
function parseSimpleTree(step, data1, data2) {
    ...
    if (step == 0) {
        ...
        for (i = 0; i < ctgr.length; i++)
            e += "#b#e#L" + i + "#Tier " + (i + 1) + " #v" + prizes[data1][i][0] + "##l\r\n";
    } else if (step == 2) {
        var reqq = reqs[data1][data2];
        for (i = 0; i < reqq.length; i++)
