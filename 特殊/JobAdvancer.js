...
function action(mode, type, selection) {
    ...
    switch (npc_state) {
        ...
        case 1:
            ...
            cm.askYesNo("Are you sure you want to Job Advance" + selection <= job[jobindex].length ? (" into a(n) #b" + job[jobindex][selection][1] + "#k") : "" + "?");
            break;
        case 2:
            if (select != 3100) {
...
function canAdvance(job, level) {
    if (level >= 10 && isBeginnerJob(job))
        return true;
    if (level >= 30 && is1stJob(job))
        return true;
    if (level >= 70 && is2ndJob(job))
        return true;
    if (level >= 120 && is3rdJob(job))
        return true;
    if (isDualBlade(job)) {
        if (level >= 20 && job == 400)
            return true;
