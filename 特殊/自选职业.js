...
var jobData = new Array(
		...
		Array("古迹猎人1转", 301, 999),
        Array("古迹猎人2转", 330, 999),
        Array("古迹猎人3转", 331, 999),
		Array("古迹猎人4转", 332, 999),
        ...
        Array("爆破手（一转）", 3700, 999),
        Array("爆破手（二转）", 3710, 999),
        Array("爆破手（三转）", 3711, 999),
        Array("爆破手（四转）", 3712, 999),
        Array("尖兵（一转）", 3600, 5),
        Array("尖兵（二转）", 3610, 5),
        Array("尖兵（三转）", 3611, 5),
        Array("尖兵（四转）", 3612, 5),
		...
		Array("魔链影士（一转）", 6400, 666),
        Array("魔链影士（二转）", 6410, 666),
        Array("魔链影士（三转）", 6411, 666),
        Array("魔链影士（四转）", 6412, 666),
		...
		Array("影魂异人（一转）", 15500, 0),
        Array("影魂异人（二转）", 15510, 0),
        Array("影魂异人（三转）", 15511, 0),
        Array("影魂异人（四转）", 15512, 0),
		Array("圣晶使徒（一转）", 15200, 0),
        Array("圣晶使徒（二转）", 15210, 0),
        Array("圣晶使徒（三转）", 15211, 0),
        Array("圣晶使徒（四转）", 15212, 0)
...
*/
    if (status == 0) {
        ...
        if (cm.getJob() % 10 == 2 && cm.getJob() != 2002 && cm.getJob() != 3002 && cm.getJob() != 4002&& cm.getJob() != 6002) { //四转不允许转职
            cm.sendOk("x"+unable);
            cm.dispose();
            return;
        } else {
            
			if (cm.getJob() % 1000 == 0 || cm.getJob() == 0 || cm.getJob() == 4001 || cm.getJob() == 4002 || cm.getJob() == 3000 || cm.getJob() == 2002 || cm.getJob() == 2003 || cm.getJob() == 2004 || cm.getJob() == 2005 || cm.getJob() == 2000 || cm.getJob() == 6001 || cm.getJob() == 3001 || cm.getJob() == 3002|| cm.getJob() == 6002|| cm.getJob() == 2003|| cm.getJob() == 15001) {
                if (cm.getJob() == 0) { //如果是法师
                    ...
                    if (cm.getJob() == 2000) {
                        ...
                        advance += "\r\n#b#L" + 6500 + "# 爆莉萌天使（一转）#l";
                    } else if (cm.getJob() == 6002) {
                        advance += "\r\n#b#L" + 6400 + "# 魔链影士（一转）#l";
                    } else if (cm.getJob() == 15001) {
                        advance += "\r\n#b#L" + 15500 + "# 影魂异人（一转）#l";
                    } else if (cm.getJob() == 301) {
                        advance += "\r\n#b#L" + 330 + "# 古迹猎人（一转）#l";
                        ...
                        advance += "\r\n#b#L" + 3101 + "# 恶魔复仇者（一转）#l";
                        //cm.dispose();
                        //cm.sendChangeJob(2159311);
                    } else if (cm.getJob() == 4001) {
                        advance += "\r\n#b#L" + 4100 + "# 剑豪（一转）#l";
                    } else if (cm.getJob() == 4002) {
                        advance += "\r\n#b#L" + 4200 + "# 阴阳师（一转）#l";
                    } else if (cm.getJob() == 14000) {
                        advance += "\r\n#b#L" + 14200 + "# 超能力者（一转）#l";
                    } else {
                        for (var i = 0; i < jobData.length; i++) {
                            if ((jobData[i][1] % 100 == 0) && (jobData[i][1] > cm.getJob()) && (jobData[i][1] < (800 + cm.getJob()))) {
                                advance += "\r\n#b#L" + jobData[i][1] + "#" + jobData[i][0] + "#l";
                            } else if (cm.getJob() == 0 & jobData[i][1] % 100 == 0 && (jobData[i][1] > cm.getJob()) && (jobData[i][1] < (800 + cm.getJob()))) {
                                advance += "\r\n#b#L" + jobData[i][1] + "#" + jobData[i][0] + "#l";
                            }
                        }
						advance += "\r\n#b#L" + jobData[27][1] + "#" + jobData[27][0] + "#l";
                    }
                } else {
                    cm.sendOk(noAdvance);
                    cm.dispose();
				...
				advance += "\r\n#b#L" + 2200 + "# 龙神（1转）#l";
			}else if(cm.getJob() == 2200&& cm.getPlayerStat("LVL") >= 30){
				advance += "\r\n#b#L" + 2211 + "# 龙神（2转）#l";
			}else if(cm.getJob() == 2211&& cm.getPlayerStat("LVL") >= 60){
				advance += "\r\n#b#L" + 2214 + "# 龙神（3转）#l";
			}else if(cm.getJob() == 2214&& cm.getPlayerStat("LVL") >= 100){
				advance += "\r\n#b#L" + 2217 + "# 龙神（4转）#l";
        ...
        cm.playerMessage(1,"经过小助手的辛苦培养。从现在开始你已经是" + newnewJobName + "了！\r\n");
        cm.dispose();
    }
}
...
function gychangejob() {
    if (status == 0) {
        if (cm.getJob() == 0 && cm.getPlayerStat("LVL") >= 10) {
            newJob = 301;
            newJobName = "古迹猎人（一转）";
        } else if (cm.getJob() == 301 && cm.getPlayerStat("LVL") >= 30) {
            newJob = 330;
            newJobName = "古迹猎人（二转）";
        } else if (cm.getJob() == 330 && cm.getPlayerStat("LVL") >= 60) {
            newJob = 331;
            newJobName = "古迹猎人（三转）";
        } else if (cm.getJob() == 330 && cm.getPlayerStat("LVL") >= 100) {
            newJob = 332;
            newJobName = "古迹猎人（四转）";
        } else {
            cm.sendOk("等级不符或已完成所有转职。");
