...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        cm.sendNextSNoESC("长老们！大家没事吧！但是……但是我们的村子……！");
    } else if (status == 1) {
        ...
        cm.sendNextSNoESC("这一切……都是我的错……虽然封印黑魔法师成功了，但是他留下的#r诅咒#k……连我们村……");
    } else if (status == 5) {
        ...
        cm.sendNextSNoESC("对不起……全都是我的错。要是我没有被黑魔法师诅咒的话……");
    } else if (status == 9) {
        ...
        cm.sendNextSNoESC("但是……我应该避免这种事情发生。也许我当初不应该去和黑魔法师战斗！……让精灵们落入了现在的境地……虽然我是国王，但我没有这样的资格！");
    } else if (status == 13) {
        ...
        cm.sendNextSNoESC("不，不是长老们的错！是我说要去和黑魔法师战斗的……我并不后悔参战。我后悔的只是没能保护好你们而已……");
    } else if (status == 17) {
        ...
        cm.sendNextSNoESC("是的……我们一定要坚持下去！");
    } else if (status == 30) {
        cm.sendNextNoESC("那当然。啊……我也无法抵挡住诅咒的力量了。快去把村子封印起来吧，双弩精灵。让我们在漫长的岁月中沉睡，等待诅咒解开的时候，不要有肮脏的人来玷污我们美丽的村庄……");
    } else if (status == 31) {
        cm.sendNextNoESC("在封印之前，需要做几样准备。你最好先去问问#b阿斯迪拉#k。");
    } else if (status == 32) {
        cm.topMsg("点击头上有灯泡图标的NPC就能开始任务。");
