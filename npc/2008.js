...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		if (questDat > 0) {
			...
			//cm.warp(50000);
			//cm.openNpc(2008, 2);
			cm.worldSpouseMessage(0x24, "『新手礼包』：【" + cm.getChar().getName() + "】 成功领取" + cm.getServerName() + " 的新人大礼包...");
			cm.worldSpouseMessage(0x24, "『新手礼包』：【" + cm.getChar().getName() + "】 成功领取" + cm.getServerName() + " 的新人大礼包...");
			cm.getQuestRecord(99997).setCustomData(1);
			//cm.openNpc(2008,3);
			cm.dispose();
			cm.sendOk("礼物已发送到你的背包里。");
        }
    }
    //cm.dispose();
}