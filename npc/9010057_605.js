...
function action(mode, type, selection) {
    ...
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            var selStr = head + "今天在线的时间为：#r" + cm.getGamePoints() + "#k 分钟! 想选择什么样的日常任务:\r\n";
            //selStr +="您当前绑定奇幻国度币：#r"+cm.getPlayerPoints()+"#k 点   \r\n\r\n";
            ...
            if (typed == 3) {
                if (selection == 2) {
                    if (cm.getEventCount("抽奖") == 0) {
                        if (cm.getGamePoints() >= 10 && cm.getEventCount("跳跃") > 0 && (cm.getSpace(1) > 1 || cm.getSpace(2) > 1 || cm.getSpace(3) > 1 || cm.getSpace(4) > 1)) {
                            cm.gainItem(2430070, 1);
                            cm.sendOk("获得 #v2430070# #t2430070# 1个");
                            cm.worldSpouseMessage(0x20, "[日常活动] 恭喜玩家 " + cm.getChar().getName() + " 在天天爱抽奖活动中获得 蓝宝石箱子 。");
                            cm.setEventCount("抽奖");
                            cm.dispose();
                        } else {
            ...
            if (typed == 5) {
                if (selection == 2) {
                    if (cm.getEventCount("历练") == 0) {
                        if (cm.getGamePoints() >= 20 && cm.getEventCount("皇陵") > 0 && cm.getEventCount("罗朱") > 0 && cm.getEventCount("海盗") > 0 && cm.getEventCount("鬼节") > 0 && (cm.getSpace(1) > 1 || cm.getSpace(2) > 1 || cm.getSpace(3) > 1 || cm.getSpace(4) > 1)) {
                            if (random == 0) {
                            ...
                            cm.worldSpouseMessage(0x20, "[日常活动] 恭喜玩家 " + cm.getChar().getName() + " 在天天爱历练活动中获得 20 点活力值 春节剪纸 x1。");
                            cm.dispose();
                        } else {
