...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (cm.getPlayer().getClient().getChannel() == 1&&2&&3&&4) {
            if (cm.getPlayer().getMapId() == 970072300) { //传送
                cm.askMenu("#e<Boss - 希-纳斯>#n\r\n你现在确定放弃任务,从这里出去?\r\n#L2##b是的,现在就出去#l");
            } else {
                ...
                zyms += "#L1##b是的,我现在就去副本#l\r\n\r\n";
                cm.askMenu(zyms);
            }
        } else {
            cm.sendOk("当前副本只能在1,2,3,4频道进行。");
            cm.dispose();
        }
