    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            if (cm.getMap().getId() == 932200002) {
				...
				status = 3;
				cm.askYesNo("这里 可以带你进入跑旗赛等候地图哦，现在进入吗？");
            } else {
                em = cm.getEventManager("biwu");
                if (em.getProperty("gate") == "0") {//已经关闭入口了
                    if (cm.getPlayer().isGM()) {
                        status = 2;
                        cm.askYesNo("尊敬的管理员，您想开放 跑旗赛 活动的入口吗？");
                    } else {
            ...
            if (selection == 0) {
                ...
                cm.sendOk("已经关闭了入口！");
                cm.spouseMessage(0x23, "[家族跑旗赛活动] 现在管理员已经关闭了活动入口。");
                //cm.worldBrodcastEffect(5121053, "[家族活动] 现在管理员已经关闭了活动入口。");
                cm.dispose();
            } else if (selection == 4) {
                em.setProperty("gate", "1");
                cm.sendOk("已经开启入口！");
                cm.spouseMessage(0x23, "[家族跑旗赛活动] 现在管理员已经开启了活动入口。");
		...
		var em = cm.getEventManager("biwu");
            ...
            em.setProperty("gate", "1");
            cm.sendOk("已经开启了入口！");
            cm.spouseMessage(0x23, "[家族跑旗赛活动] 管理员已经开放了活动入口，请大家速度从拍卖处的副本入口进来哦！");
