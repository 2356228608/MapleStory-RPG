...
function action(mode, type, selection) {
    ...
    if (cm.getInfoQuest(23007).indexOf("vel01=2") == -1 && cm.getInfoQuest(23007).indexOf("vel01=3") == -1) {
        if (status == 0) {
            ...
            cm.sendNextPrevS("好了，现在没有东西拦着你了？我们一起走吧！");
        } else if (status == 2) {
            cm.sendNextPrev("但，但是……");
        } else if (status == 3) {
            cm.sendNextPrevS("你想继续留在这里吗？");
        } else if (status == 4) {
            cm.sendNextPrev("怎么可能！我不想一辈子做实验体！");
        } else if (status == 5) {
            cm.sendNextPrevS("那我们就一起逃走吧！快！");
        } else if (status == 6) {
        ...
        if (status == 0) {
            cm.sendNext("好久没到实验室外面来了……这是什么地方？");
        } else if (status == 1) {
            cm.sendNextPrevS("这是通往我们的村子埃德尔斯坦的路！在刚才那些奇怪的黑色之翼跟来之前，我们快走吧。");
        } else if (status == 2) {
        ...
        cm.sendOk("我真的好久没到实验室外面来了……这是什么地方？");
        cm.dispose();
    }
}