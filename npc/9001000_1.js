...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getPlayerStat("GM") != 4) {
            //cm.askMenu("亲爱的管理员#r#h ##k您好，您有什么事情需要我帮忙的吗?\r\n#b\r\n#L2#修改装备属性#l\r\n#L3#查看潜能属性#l\r\n#L4#初始化AP/SP#l\r\n#L5#清除技能#l\r\n#L6#加满职业技能#l\r\n#L7#初始化属性\r\n");  

            cm.askMenu("亲爱的管理员#r#h ##k您好，您有什么事情需要我帮忙的吗?\r\n#b\r\n#L2#修改装备属性#l\r\n");

            //#L1#加满属性#l\r\n
        } else if (cm.getPlayerStat("GM") != 4) {
            cm.askMenu("亲爱的管理员#r#h ##k您好，您有什么事情需要我帮忙的吗?#b\r\n#L32#装备一键上锁#l");
        } else {
            cm.dispose();
        }
    } else if (status == 1) {
        if (selection == 0) {
            ...
            cm.dispose();
        } else if (selection == 2 && cm.getPlayerStat("GM") != 4) {
            var avail = "";
            for (var i = -1; i > -199; i--) {
            ...
            cm.askMenu("请选择您需要修改属性的装备:\r\n#b" + avail);
        } else if (selection == 3 && cm.getPlayerStat("GM") != 4) {
            ...
            cm.askMenu("What would you like to learn about?\r\n#b" + avail);
            status = 9;
        } else if (selection == 4) {
    ...
    } else if (status == 2 && cm.getPlayerStat("GM") != 4) {
        selected = selection - 1;
        var text = "";
        for (var i = 0; i < stats.length; i++) {
            text += "#L" + i + "#" + stats[i] + "#l\r\n";
        }
        cm.askMenu("你选择了修改 #b#t" + cm.getInventory(-1).getItem(slot[selected]).getItemId() + "##k 装备的属性\r\n请选择修改这个装备的具体属性\r\n#b" + text);
    } else if (status == 3 && cm.getPlayerStat("GM") != 4) {
        statsSel = selection;
        cm.playerMessage("当前选择 " + selection + "  " + slot[selected]);
        if (selection == 20) {
            cm.askText("What would you like to set your #b#t" + cm.getInventory(-1).getItem(slot[selected]).getItemId() + "##k's " + stats[statsSel] + " to?");
        } else {
            cm.askNumber("What would you like to set your #b#t" + cm.getInventory(-1).getItem(slot[selected]).getItemId() + "##k's " + stats[statsSel] + " to?", 0, 0, 32767);
        }
    } else if (status == 4 && cm.getPlayerStat("GM") != 4) {
        ...
        cm.dispose();
    } else if (status == 10 && cm.getPlayerStat("GM") != 4) {
        if (selection == 0) {
            cm.askText("What would you like to search for? (e.g. STR %)");
            return;
        }
        cm.askMenu("#L3#" + cm.getPotentialInfo(selection) + "#l");
        status = 0;
    } else if (status == 11 && cm.getPlayerStat("GM") != 4) {
