...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (a == -1) {
            ...
            if (selection == 2014) {
                a = 0;
                cm.askText("请输入您要搜索的点装名称，可以进行模糊查询");
                isSearch = true;
            } else {
                typed = selection;
                if (isSearch) {
                ...
                for (var i = 0; i < lastItemList.length; i++) {
                    if (getItemType(lastItemList[i][0]) != typeList[typed][0] && !isSearch)
                        continue;
                    text += "#L" + (i) + "##i" + lastItemList[i] + ":##t" + lastItemList[i] + "# - #r" + lastItemList[i][1] + "#b蜗牛邮票  \r\n";
                }
                isSearch = false;
                cm.askMenu(text);
            }
            ...
            selects = selection;
            buynum = 1;
            cm.askYesNo("你想购买" + buynum + "个#r#i" + lastItemList[selects][0] + "##k？\r\n你将使用掉" + (buynum * lastItemList[selects][1]) + "蜗邮牛票。");
        } else if (a == 3) {
            if (cm.getSpace(1) < 1) {
                cm.sendOk('背包栏位不足');
                cm.dispose();
