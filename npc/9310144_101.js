...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1) {
            ...
            if (status == 0) {
                cm.sendNext("如果您需要点卷蜗牛邮票的话，那么请下次来找我！");
                cm.dispose();
            }
            status--;
        }
        if (status == 0) {
            cm.askMenu("#r#v4002001##v4002001##v4002001##v4002001##v4002001##v4002001##v4002001##v4002001##v4002001##v4002001##v4002001##k#l\r\n点卷:#r " + cm.getPlayer().getCSPoints(1) + " #k点  蜗牛邮票:#r " + cm.getItemQuantity(4002001) + " #k个\r\n    #b#L0#我要兑换物品   (1000点卷=1个蜗牛票)#l   \r\n    #L1#我要兑换点卷   (1个蜗牛邮票=1000点卷)#l   \r\n\r\n #e#d说明：我们的蜗牛邮票可以放在雇佣商店里面买卖。\r\n\r\n#r#v4002001##v4002001##v4002001##v4002001##v4002001##v4002001##v4002001##v4002001##v4002001##v4002001##v4002001#");
        } else if (status == 1) {
            ...
            if (selection == 0) {
                if (cm.getPlayer().getCSPoints(1) / 1000 == 0) {
                    cm.sendNext("您的点卷不足，无法兑换蜗牛邮票。");
                    status = -1;
                } else {
                    beauty = 1;
                    cm.askNumber("请输入点卷兑换蜗牛票的数量:\r\n兑换比例为 1000 : 1\r\n", 1, 1, cm.getPlayer().getCSPoints(1) / 1000);
                }
            } else if (selection == 1) {
            ...
            if (beauty == 1) {
                ...
                if (cm.getSpace(4) < sl) {
					...
					cm.dispose();
                    cm.sendOk("您成功将#r " + (selection * 1000) + " #k点卷换为蜗牛邮票#v4002001# x #r" + selection + " #k")
                    //cm.worldSpouseMessage(0x11, "[蜗牛证券] 玩家 " + cm.getChar().getName() + " 成功将 " + (selection * 1000) + " 点卷换为 " + selection + " 个蜗牛票。");
                } else {
                    cm.sendNext("您的输入的数量错误，无法兑换蜗牛邮票。");
                    cm.dispose();
                }
            } else if (beauty == 2) {
                if (cm.haveItem(4002001, selection)) {
                    ...
                    cm.dispose();
					cm.sendOk("您成功将蜗牛邮票#v4002001# x #r" + selection + " #k换为#r " + (1000 * selection) + " #k点卷。");
                    //cm.worldSpouseMessage(0x20, "[蜗牛证券] 玩家 " + cm.getChar().getName() + " 成功将" + selection + " 个蜗牛邮票 换为 " + (1000 * selection) + " 点卷。");
                } else {
