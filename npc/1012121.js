...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            status++;
        if (status == 0) {
            cm.askMenu("你好伟大的#b#h ##k，我是本服管理装备的NPC，#r需要注意一点是，清理的装备是不能找回的，为避免给你带来不必要的麻烦，所以请谨慎使用#k\r\n#L1##e#r<慎重>#b清空背包垃圾(选择性删除)#b#l\r\n#L245##e#r<慎重>#b清空背包垃圾(一次性删除)#b#l\r\n#L105##e#r<适合所有玩家>属性置换(新)#b#l\r\n");
        } else if (status == 1) {
            if (selection == 0) { //正义币
                fstype = 1;
                ...
                fstype = 2;
                cm.askMenu("目前只受理#r装备栏#k和#r现金栏#k两个栏位的清理工作，主要考虑到部份玩家提出的无法清理点装的问题，没有地方放，#r如果你不想要某个装备，请选择后，在删除~是不是很方便？注意，此处清理为不可恢复性清理，所以在准备清理前一定看清楚了.\r\n#b#L1#我要考虑清理装备栏某一个装备#l\r\n#L2#我要考虑清理现金栏某一个装备#l");
            ...
            if (fstype == 16) {
                ...
                if (chance <= 5) { //watk
                    ...
                    Packages.server.MapleInventoryManipulator.addFromDrop(cm.getC(), item, false);
                    cm.sendOk("恭喜，成功给装备增加了:#r" + lvsj + "#k攻击.");
                    cm.worldMessage("[装备升级]：恭喜[" + cm.getChar().getName() + "]在市场Kin处，使用哈勒沐币叶提升了武器的攻击");
                } else if (chance > 5 && chance <= 20) { //matk
                    item.setMatk(item.getMatk() * 1 + lvsj);
                    Packages.server.MapleInventoryManipulator.removeFromSlot(cm.getC(), Packages.client.inventory.MapleInventoryType.EQUIP, 1, 1, false);
                    Packages.server.MapleInventoryManipulator.addFromDrop(cm.getC(), item, false);
                    cm.sendOk("恭喜，成功给装备增加了:#r" + lvsj + "#k魔攻.");
                    cm.worldMessage("[装备升级]：恭喜[" + cm.getChar().getName() + "]在市场Kin处，使用哈勒沐币叶提升了武器的魔法攻击");
                } else if (chance > 20 && chance <= 40) { //str
                    item.setStr(item.getStr() * 1 + lvsj);
                    Packages.server.MapleInventoryManipulator.removeFromSlot(cm.getC(), Packages.client.inventory.MapleInventoryType.EQUIP, 1, 1, false);
                    Packages.server.MapleInventoryManipulator.addFromDrop(cm.getC(), item, false);
                    cm.sendOk("恭喜，成功给装备增加了:#r" + lvsj + "#k力量.");
                    cm.worldMessage("[装备升级]：恭喜[" + cm.getChar().getName() + "]在市场Kin处，使用哈勒沐币叶提升了武器的力量");
                } else if (chance > 40 && chance <= 60) { //dex
                    item.setDex(item.getDex() * 1 + lvsj);
                    Packages.server.MapleInventoryManipulator.removeFromSlot(cm.getC(), Packages.client.inventory.MapleInventoryType.EQUIP, 1, 1, false);
                    Packages.server.MapleInventoryManipulator.addFromDrop(cm.getC(), item, false);
                    cm.sendOk("恭喜，成功给装备增加了:#r" + lvsj + "#k敏捷.");
                    cm.worldMessage("[装备升级]：恭喜[" + cm.getChar().getName() + "]在市场Kin处，使用哈勒沐币叶提升了武器的敏捷");
                } else if (chance > 60 && chance <= 80) { //luk
                    item.setInt(item.getInt() * 1 + lvsj);
                    Packages.server.MapleInventoryManipulator.removeFromSlot(cm.getC(), Packages.client.inventory.MapleInventoryType.EQUIP, 1, 1, false);
                    Packages.server.MapleInventoryManipulator.addFromDrop(cm.getC(), item, false);
                    cm.sendOk("恭喜，成功给装备增加了:#r" + lvsj + "#k智力.");
                    cm.worldMessage("[装备升级]：恭喜[" + cm.getChar().getName() + "]在市场Kin处，使用哈勒沐币叶提升了武器的智力");
                } else if (chance > 80) { //int
                    item.setLuk(item.getLuk() * 1 + lvsj);
                    Packages.server.MapleInventoryManipulator.removeFromSlot(cm.getC(), Packages.client.inventory.MapleInventoryType.EQUIP, 1, 1, false);
                    Packages.server.MapleInventoryManipulator.addFromDrop(cm.getC(), item, false);
                    cm.sendOk("恭喜，成功给装备增加了:#r" + lvsj + "#k运气.");
                    cm.worldMessage("[装备升级]：恭喜[" + cm.getChar().getName() + "]在市场Kin处，使用哈勒沐币叶提升了武器的运气");
                }
                cm.dispose();
                return;
            }
            if (fstype == 18) {
                if (cm.getHyPay(1) < 8888) {
                    cm.sendOk("对不起,你没有足够的哈勒沐币");
                    cm.dispose();
                    return;
                }
                cm.addHyPay(8888, true);
