...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (job == 0 || job == 2000 || job == 1000) {
            cm.askYesNo("We have a special 90% discount for beginners. The Ant Tunnel is located deep inside in the dungeon that's placed at the center of the Victoria Island, where the 24 Hr Mobile Store is. Would you like to go there for #b1,000 mesos#k?");
            cost = 1000;
        } else {
            cm.askYesNo("The regular fee applies for all non-beginners. The Ant Tunnel is located deep inside in the dungeon that's placed at the center of the Victoria Island, where 24 Hr Mobile Store is. Would you like to go there for #b10,000 mesos#k?");
            cost = 10000;
        }
    } else if (status == 2) {
        if (cm.getMeso() < cost) {
