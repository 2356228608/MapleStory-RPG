var status = 0;
var maps = Array(102000000, 101000000, 100000000, 103000000, 12000000000, 105000000);
...
function action(mode, type, selection) {
    if (status >= 27 && mode == 0) {
        cm.sendNext("如果你想到其他村子去，欢迎随时来乘坐出租车～");
        cm.dispose();
        return;
    ...
    if (mode == 1)
        ...
        status--;
    if (status == 0) {
        cm.sendNext("你好～！我是#p1002000#。你想快速安全地移动到其他村庄吗？那样的话，请乘坐为顾客提供最好服务的#b#p1002000##k吧。我们可以以很便宜的价格，将你送到想去的地方。");
    } else if (status == 1) {
        cm.askMenu("好的,选择您想要进行的项目吧。\r\n#L0##b告诉我村庄的故事#l\r\n#L1#我想前往村庄#k#l");
    } else if (status == 2) {
        if (selection == 0) {
            cm.askMenu("选择您想要了解的村庄:\r\n#b#L0##m104000000##l\r\n#L1##m102000000##l\r\n#L2##m101000000##l\r\n#L3##m100000000##l\r\n#L4##m103000000##l\r\n#L5##m400000##l\r\n#L6##m105040300##l");
        } else if (selection == 1) {
            status = 26;
            if (cm.getJob() == 0) {
                var selStr = "新手玩家可以获得#b90%#k的折扣。请选择目的地。#b";
        ...
        if (selection == 0) {
            ...
            status = 19;
            cm.sendNext("Here's a little information on #b#m400000##k. It's a submarine that's currently parked in between Ellinia and Henesys in Victoria Island. That submarine serves as home to numerous pirates. You can have just as beautiful a view of the ocean there as you do here in Lith Harbor.");
        } else if (selection == 6) {
            status = 22;
        ...
        cm.sendNextPrev("#m400000# is parked in between Henesys and Ellinia, so if you step out just a bit, you'll be able to enjoy the view of both towns. All the pirates you'll meet in town are very gregarious and friendly as well.");
    } else if (status == 21) {
        cm.sendNextPrev("If you are serious about becoming a #bPirate#k, then you better meet the captain of #m400000#, #r#p1090000##k. If you are over Level 10 with 20 DEX, then she may let you become one. If you aren't up to that level, then you'll need to train harder to get there!");
    } else if (status == 22) {
        cm.sendNext("Alright I'll explain to you more about #bSleepywood#k. It's a forest town located at the southeast side of Victoria Island. It's pretty much in between Henesys and the ant-tunnel dungeon. There's a hotel there, so you can rest up after a long day at the dungeon ... it's a quiet town in general.");
    } else if (status == 23) {
        ...
        cm.askYesNo("I guess you don't need to be here. Do you really want to move to #b#m" + maps[selection] + "##k? Well it'll cost you #b" + sCost + " mesos#k. What do you think?");
