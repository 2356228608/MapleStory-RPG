...
function action(mode, type, selection) {
    if (mode == 0 && status == 0) {
        ...
        if (status == 0) {
            ...
            if (section < 2) {
                cm.askMenu("Hello. Would you like to buy a ticket for the subway?\r\n#L0##b" + msg[section] + "#l");
            } else {
                cm.askYesNo("Do you want to go back to " + msg[section] + " subway station now?");
            }
        } else if (status == 1) {
            if (section < 2) {
                cm.askYesNo("The ride to " + msg[section] + " takes off every 10 minutes, beginning on the hour, and it'll cost you #b" + cost + " mesos#k. Are you sure you want to purchase #b#t" + ticket[section] + "##k?");
            } else {
                section -= 2;
