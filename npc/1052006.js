...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getPlayerStat("LVL") <= 19) {
            ...
            cm.askMenu("You must purchase the ticket to enter. Once you have made the purchase, you can enter through The Ticket Gate on the right. What would you like to buy?" + menu);
        }
    } else if (status == 1) {
        selector = selection;
        selection += 1;
        cm.askYesNo("Will you purchase the ticket to #bConstruction Site B" + selection + "#k? It'll cost you " + meso[selector] + " mesos. Before making the purchase, please make sure you have an empty slot on your etc. inventory.");
    } else if (status == 2) {
