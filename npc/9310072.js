...
var mhair2 = Array(36704, 40390, 40470, 40344, 40490, 40433, 40000, 40010, 40020, 40030, 40040, 40050, 40060, 40090, 40100, 40110, 40250, 40260, 40280, 40350, 40360, 40370, 40390, 41060, 41070, 42060, 35290, 35710, 35720, 35770, 35800, 35440, 35430, 35500, 35350, 35340, 35600, 35630, 35690, 35750, 35730, 35721, 35712, 35705, 35693, 35550, 35084, 35063, 35211, 35360, 35330, 35460, 35440, 35430, 35560, 35602, 36340, 35090, 35080, 35160, 35150, 35130, 35120, 35110, 35100, 35190, 35240, 35230, 35220, 35350, 36130, 36160, 36140, 36170, 36480, 36470, 36460, 36450, 36620);//男发型 ,36630,36640,36650,36670,36690,36710,36730,36740,36750,36760,36770,36800,36820,36840,36860,36890,36900,36920,36930,36980,36940,36990,33380,33750,33790,34370,34940,35000,35010,35020,35030,35040,35050,35070,37690,37890,31870,32470,32490,33645,35060,35180,35200,35210,35260,35340,36230,36680,36490,36810,36830,36880,36950,37120,36700
...
var hairnew = Array();
var mface = Array(24013, 23281, 23599, 23573, 23783, 23831, 23033, 25006, 25300, 26004, 23076, 23018, 23068, 23078, 23077, 23086, 23073, 20035, 20147, 20046, 20462, 20377, 23071, 20092, 20091, 23043, 23042, 23041, 24016, 20044, 20038, 20047, 20098, 23059, 24057, 23053, 21099, 23057, 23033, 23034, 23029, 23030, 24025, 24015, 23025, 23019, 23014, 23024, 23017, 23015, 23110, 23021, 23016, 24011, 24012, 20099, 24001, 23004, 21092, 20000, 20001, 20002, 20003, 20004, 20005, 20006, 20007, 20008, 20009, 20010, 20011, 20012, 20013, 20014, 20015, 20016, 20017); //男脸 , 20018, 20019, 20020, 20021, 20022, 20023, 20024, 20025, 20026, 20027, 20028, 20029, 20036, 20045, 20037, 20040, 20048, 20049, 20050, 20051, 20052, 20053, 20055, 20056, 20058, 20059, 20060, 20062, 20063, 20064, 20065, 20066, 20068, 20067, 20069
var fface = Array(21031, 21042, 24012, 24016, 24051, 24082, 24097, 24071, 24081, 24871, 25006, 25300, 26004, 26403, 26507, 26805, 26806, 24071, 24063, 24090, 24077, 24079, 24014, 24051, 24008, 24029, 24030, 24025, 24066, 24082, 24081, 24088, 24067, 21028, 24050, 24057, 23053, 23054, 23064, 23060, 24058, 24020, 24013, 24015, 23831, 24126, 23028, 24070, 24168, 24059, 23060, 23064, 23153, 23059, 23753, 23056, 23057, 24056, 24052, 24035, 24041, 24040, 24039); //女脸 , 21001, 21002, 21003, 21004, 21005, 21006, 21007, 21008, 21009, 21010, 21011, 21012, 21013, 21014, 21015, 21016, 21017, 21018, 21019, 21020, 21021, 21022, 21023, 21024, 21025, 21026, 21027, 21034, 21043, 21029, 21030, 21038, 21046, 21047, 21048, 21049, 21050, 21052, 21053, 21054, 21055, 21056, 21057, 21058, 21059, 21060, 21061, 21062, 21064,24038,24036,23034,23040,24032,24031,21036,24028,23808,24025,23030,23029,24722,23032,21444,21131,21033,21145,21175,21180,21179,21177,21178,21193,21065,21000
var facenew = Array();
    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (selection == 0) {
                ...
                cm.askAvatar("我可以改变你的脸型,让它比现在看起来漂亮. 你为什么不试着改变它下? ,我将会帮你改变你的脸型,那么选择一个你想要的新脸型吧!", facenew, 5152001);
            } else if (selection == 1) {
                beauty = 1;
                if (cm.getChar().getGender() == 0) {
                ...
                cm.askAvatar("请选择你喜欢的颜色.", colors, 5152001);
            } else if (selection == 2) {
                beauty = 2;
                haircolor = Array();
                ...
                cm.askAvatar("我可以改变你的发色,让它比现在看起来漂亮. 你为什么不试着改变它下? 我将会帮你改变你的发色,那么选择一个你想要的新发色吧!", haircolor, 5151001);
                ...
                cm.askAvatar("我可以改变你的发型,让它比现在看起来漂亮.你为什么不试着改变它下? 我将会帮你改变你的发型,那么选择一个你想要的新发型吧!", hairnew, 5150001);
            } else if (selection == 4) {
                beauty = 4;
                cm.askAvatar("用我们特殊开发的机器可查看护肤后的效果噢,想换成什么样的皮肤呢？请选择～~", skin, 5153000);
            } else if (selection == 6) {
                cm.dispose();
                cm.openNpc(1012117);
            } else if (selection == 7) {
            ...
            if (beauty == 0) {
                if (cm.haveItem(5152001) == 1) {
					...
					cm.dispose();
                    cm.playerMessage(1,"好了,你的朋友们一定认不出来是你了!");
                ...
                if (cm.haveItem(5152001) == 0) {
                    cm.gainItem(5152001, -0);
                    cm.getChar().setFace(colors[selection]);
					cm.dispose();
                    cm.playerMessage(1,"好了,你的朋友们一定认不出来是你了!");
                } else if (cm.getPlayer().getCSPoints(1) >= 0) {
                    cm.getPlayer().modifyCSPoints(1, -0);
