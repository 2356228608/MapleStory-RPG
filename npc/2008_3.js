        ...
        if (typed == 0) {
            ...
            if (isExists) {
                ...
                if (isSuccess) {
                    ...
                    cm.setBossLogAcc("推广设置", -2);
                    cm.askMenu("您已经设置过邀请者的账号，无法重复设置。");
                    status = -1
                }
            } else {
                cm.askMenu("#r该账号不存在，无法设置。#k");
                status -= 2;
            }
        }
    }
...
var Invitation = function () {
    ...
    this.getInvitation = function () {
        ...
        if (rs.next())
            this.invitation = rs.getString("invitation");
        rs.close();
        pstmt.close();
        return this.invitation;
    }
