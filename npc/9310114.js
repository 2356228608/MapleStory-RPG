...
function start() {
    if (cm.getPlayer().getMapId() == 980000503) {
        cm.dispose();
        cm.openNpc(9310114, 1);
        return;
    }
    status = -1;
    action(1, 0, 0);
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        cm.askYesNo("您真的打败了生化魔人欧啦啦么。  请务必获取欧啦啦掉落物 #r#t4001318##k，我会赠送您一份小礼物，请务必笑纳。");
    } else if (status == 1) {
        if (cm.haveItem(4001318, 1) == false) {
            ...
            cm.worldSpouseMessage(0x0C, "『心动的公主』" + " : " + "[" + cm.getChar().getName() + "]获得了" + psrw[rand][1] + "个<" + ii.getName(psrw[rand][0]) + ">,快去挑战吧");
            cm.dispose();
        }
    }
}
