...
function action(mode, type, selection) {
    if (cm.getQuestStatus(21002) == 0) {
        ...
        if (status == 0) {
            ...
            cm.effect_Direction("Effect/Direction1.img/aranTutorial/Trio");
            cm.dispose();
        }
    } else {
        if (mode == 1) {
        ...
        if (status == 0) {
            cm.askMenu("We're in a state of emergency. What would you like to know? \r #b#L0#Where's the Black Wizard?#l \r #b#L1#How's the preparation for the escape?#l \r #b#L2#How about the comrades?#l");
        } else if (status == 1) {
            switch (selection) {
                case 0:
