...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            cm.askMenu("初次见面～我是在#m101000000#研究多种魔法的#p1032102#，我特别感兴趣的是有关生命的魔法。生命多么神秘呀...我正在研究怎么创造那样生命。#b\r\n#L0#我要把宝贝龙进化#l\r\n#L1#我要把罗伯进化#l\r\n#L2#我要把我的宠物复活#l#k");
        } else if (status == 1) {
            if (selection == 0) {
                var currentpet = null;
                for (var i = 0; i < 3; i++) {
                ...
                if (pet == null || !cm.haveItem(5380000, 1)) {
					...
					cm.gainPetItem(after, 1);
                    cm.getPlayer().spawnPet(slot);
                    cm.sendOk("你的龙已经进化了！过去是一个 #i" + id + "##t" + id + "#, 现在是一个 #i" + after + "##t" + after + "#!");
                    cm.dispose();
                }
            } else if (selection == 1) {
                var currentpet = null;
