...
function action(mode, type, selection) {
    ...
    if (status == 0) { // first interaction with NPC
        ...
        cm.askYesNo("The deal is simple. You get me something I need, I get you something you need. The problem is, I deal with a whole bunch of people, so the items I have to offer may change every time you see me. What do you think? Still want to do it?");
    } else if (status == 2) {
        var eQuestChoice = makeChoices(eQuestChoices);
        cm.askMenu(eQuestChoice);
    } else if (status == 3) {
        lastSelection = selection;
        requiredItem = eQuestChoices[selection];
        cm.askYesNo("Let's see, you want to trade your #b100 #t" + requiredItem + "##k with my stuff right? Before trading make sure you have an empty slot available on your use or etc. inventory. Now, do you want to trade with me?");
    } else if (status == 4) {
