...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        cm.askMenu("Hello there~ I am #p" + cm.getNpc() + "# from Singapore Airport. I will assist you in getting back to #m103000000# in no time. How can i help you?\r\n#L0##bI would like to buy a plane ticket to #m103000000##k#l\r\n#L1##bLet me go in to the departure point.#k#l");
    } else if (status == 1) {
        menu = selection;
        if (menu == 0) {
            cm.askYesNo("The ticket will cost you 20,000 mesos. Will you purchase the ticket?");
        } else if (menu == 1) {
            cm.askYesNo("Would you like to go in now? You will lose your ticket once you go in~ Thank you for choosing Wizet Airline.");
        }
    } else if (status == 2) {
        if (menu == 0) {
