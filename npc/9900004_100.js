...
//图标： + icon + ;   + iconl + 
            ...
            if (typed == 111) {
                switch (selection) {
                    case 111:
                        typed = 1111;
                        cm.askNumber("请输入你要买入的股票，购买比例是#r" + cm.getStockPrice(1) / 100 + "#k股票币=#r1股娜娜有限公司#k。每次最多可以买入500股。#k", 0, 0, 500);
                        break;
                    case 222:
                        typed = 2222;
                        cm.askNumber("请输入你要买入的股票，购买比例是#r" + cm.getStockPrice(2) / 100 + "#k股票币=#r1股喵喵有限公司#k。每次最多可以买入500股。#k", 0, 0, 500);
                        break;
                    case 333:
                        typed = 3333;
                        cm.askNumber("请输入你要买入的股票，购买比例是#r" + cm.getStockPrice(3) / 100 + "#k股票币=#r1股婷婷有限公司#k。每次最多可以买入500股。#k", 0, 0, 500);
                        break;
                }
            }
            if (typed == 222) {
                switch (selection) {
                    case 111:
                        typed = 11111;
                        cm.askNumber("请输入你要卖出的股票，卖出比例是1股娜娜有限公司=" + cm.getStockPrice(1) / 100 + "#k股票币#k。每次最多可以卖出500股。#k", 0, 0, 500);
                        break;
                    case 222:
                        typed = 22222;
                        cm.askNumber("请输入你要卖出的股票，卖出比例是1股喵喵有限公司=" + cm.getStockPrice(2) / 100 + "#k股票币#k。每次最多可以兑换500股。#k", 0, 0, 500);
                        break;
                    case 333:
                        typed = 33333;
                        cm.askNumber("请输入你要卖出的股票，卖出比例是1股婷婷有限公司=" + cm.getStockPrice(3) / 100 + "#k股票币#k。每次最多可以兑换500股。#k", 0, 0, 500);
                        break;
                }
            }
        } else if (status == 4) {
            if (typed == 11111) {
                fee = selection;
                selectedpay = cm.getStockPrice(1) / 100 * fee;
            ...
            if (typed == 22222) {
                fee = selection;
                selectedpay = cm.getStockPrice(2) / 100 * fee;
                if (fee <= 0 || fee > 500 || fee % 1 != 0) {
            ...
            if (typed == 33333) {
                fee = selection;
                selectedpay = cm.getStockPrice(3) / 100 * fee;
                if (fee <= 0 || fee > 500 || fee % 1 != 0) {
