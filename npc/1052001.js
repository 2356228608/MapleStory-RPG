...
function action(mode, type, selection) {
    ...
    if (status == 0) {
            ...
            if (cm.getPlayerStat("LVL") >= 30 && cm.getJob() == 400 && cm.getPlayer().getSubcategory() == 0) {
                if (cm.getQuestStatus(100009) >= 1) {
                    ...
                    if (cm.getQuestStatus(100011) == 2) {
                        ...
                        cm.sendOk("Well this is weird.. Report it on the forum!")
                        cm.dispose();
                    }
                } else {
                    status = 10;
        ...
        cm.askYesNo("Do you want to become a #rThief#k?");
    } else if (status == 3) {
        if (cm.getJob() == 0) {
        ...
        cm.askMenu("What do you want to become?#b\r\n#L0#Assassin#l\r\n#L1#Bandit#l#k");
    } else if (status == 22) {
        var jobName;
        if (selection == 0) {
            jobName = "Assassin";
        ...
        cm.askYesNo("Do you want to become a #r" + jobName + "#k?");
    } else if (status == 23) {
        cm.changeJob(job);
