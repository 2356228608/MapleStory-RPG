...
function action(mode, type, selection) {
    ...
    if (cm.getMapId() == 910000000) {
        cm.dispose();
        if (status == 0) {
            cm.openNpc(1540434, "RingQustion");
        }
        return;
    } else {
        if (status == 0) {
            cm.askMenu("You want to learn more about the minigames? Awesome! Ask me anything. Which minigame do you want to know more about?\r\n#b#L0#Omok#l\r\n#b#L1#Match Cards#l");
        } else if (status == 1) {
            sel = selection;
            if (selection == 0) {
