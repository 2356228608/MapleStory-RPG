...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
			var text="亲爱的#r#h ##k您好，我是伤害上限突破系统:\r\n";
			text+="#r#L999#伤害上限突破简介#l\r\n\r\n";
			//text+="#L1#[武器]消费币突破 (#k目前状态：#r土豪推荐#b)#l\r\n";
			text+="#L2#[武器]正义火种突破 (#k目前状态：#r火爆内容#b)#l\r\n";
			text+="#L3#[武器]中介币突破   (#k目前状态：#r推荐内容#b)#l\r\n";
			text+="#L4#[武器]100000点卷突破100000伤害#l\r\n";
			text+="#L5#[武器]1亿点卷突破突破1亿伤害#l\r\n";
			text+="";
            ...
            if (selection == 999) {
                ...
                typed = selection;
                if (selection == 1) {
                    cm.askYesNo("是否花费#k50余额对当前穿戴的武器进行伤害上限突破？-[已经开放]");
                } else if (selection == 2) {
                    cm.askYesNo("是否花费3个#b正义火种1#k对当前穿戴的武器进行伤害上限突破？");
                } else if (selection == 3) {
                    cm.askYesNo("是否花费50个#b中介币#k对当前穿戴的武器进行伤害上限突破？");
                }else if (selection == 4) {
                    cm.askYesNo("是100000点卷#k对当前穿戴的武器进行伤害上限突破？");
                }else if (selection == 5) {
                    cm.askYesNo("是1亿点卷#k对当前穿戴的武器进行伤害上限突破？");
                }
            }
        } else if (status == 2) {
            if (selection = -1)
            ...
            if (selection == 4) {
                ...
                if (cm.getHyPay(1) > 49) {
                    if (cm.changeLimitBreak(zjbNum)) {
                        ...
                        cm.addHyPay(50, true);
                        cm.sendOk("#b伤害上限突破成功.\r\n\r\n本次追加伤害为：#r" + zjbNum + "#b.");
                        cm.worldSpouseMessage(0x20, "[伤害突破] 玩家 " + cm.getChar().getName() + " 使用 [ 50 ]消费币 让武器伤害上限突破成功 本次追加 " + zjbNum + " 伤害值 。");
                    } else {
                        cm.sendOk("#b突破失败.\r\n系统为检测到角色身上装备武器.");
                    }
                    cm.dispose();
