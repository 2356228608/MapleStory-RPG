...
//cm.sendOk("祝您游戏愉快!!!");
    ...
    if (status == 0) {
        ...
        if (selection == 0) {
            ...
            var i = 0;
            var LogRecord = cm.getConnection().prepareStatement("SELECT tid,cid,boughtByCid,boughtByCName,boughtTradeType,boughtPrice,boughtItemid,DATE_FORMAT(boughtDate,'%m月%d日') as boughtDate FROM cashtradesystemLog LIMIT 100").executeQuery();
        ...
        if (selection == 99999) {
            ...
            while (AllRecordAll.next()) {//得到记录数据
				...
				insertEquipData.setString(45, AllRecordAll.getString("state"));
                insertEquipData.setString(46, AllRecordAll.getString("bonusState"));
            ...
            while (AllRecords.next()) {//得到记录数据
                if (AllRecords.getString("id") == TradeId) {
					...
					insertEquipData.setString(45, AllRecords.getString("state"));
                	insertEquipData.setString(46, AllRecords.getString("bonusState"));
                    insertEquipData.executeUpdate(); //更新
                }
            }
        ...
        while (LogRecord.next()) {//得到记录数据
            ...
            if (LogRecord.getString("boughtTradeType") == 0) {//余额
