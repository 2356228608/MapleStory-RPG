...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (cm.getPlayer().getClient().getChannel() == 1 || cm.getPlayer().getClient().getChannel() == 2) {
            ...
            cm.sendOk("该副本只在1、2线进行。");
            cm.dispose();
            return;
        }
        if (cm.getPlayer().getMapId() != 744000000) { //传送
            cm.askMenu("#e#v3991051##v3991051##v3991051##v3991051#<Boss - 枫之高校>#v3991050##v3991050##v3991050##v3991050##n\r\n你现在确定放弃任务,从这里出去?\r\n#L2##b是的,现在就出去#l");
            ...
            rwpz = "经验指数：#r★★★★★#k\r\n";
            rwpz += "#e推荐等级：150 - 250";
            rwpz += "        推荐人数：1 - 3  \r\n#b已进行" + bossLevel + "：" + cm.getBossLog(PQ) + " 次       剩余挑战次数：" + pqtry + " 次#k";
            rwpz += "\r\n" + bossLevel + "状态：" + vipstr + "        #n";
            //	rwpz += "\r\n1、2线为普通模式，3、4线为困难模式。";
            var zyms = "";
            zyms = "   #fUI/UIWindow2.img/StagedGachapon/Creature/0/normal/2#   #fUI/UIWindow2.img/Megaphone/0##fUI/UIWindow2.img/Megaphone/0##r <Boss - 枫之高校> #fUI/UIWindow2.img/Megaphone/0##fUI/UIWindow2.img/Megaphone/0#   #fUI/UIWindow2.img/StagedGachapon/Creature/0/normal/2##n\r\n#b#h0# \n\#k你现在想挑战这个BOSS副本吗?\r\n" + rwpz + "\r\n";
