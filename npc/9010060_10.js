...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            cm.askMenu("#fMob/1210102.img/move/0##fMob/1210102.img/move/0##b追忆 - MS#fMob/1210102.img/move/0##fMob/1210102.img/move/0##k\r\n\r\n您目前身上有#r " + cm.getMeso() + " #k金币\r\n银行存款为#r " + cm.getMoney() + " #k亿金币\r\n#b#L0#我要存款#l\r\n#L1#我要取款#l");
        } else if (status == 1) {
            selectedType = selection;
            if (selectedType == 0) {
                cm.askNumber("请输入您要存款的金额(单位: 亿 ):\r\n", 1, 1, 23);
            } else if (selectedType == 1) {
                cm.askNumber("请输入您要取款的金额(单位: 亿 ):\r\n", 1, 1, 5);
            }
        } else if (status == 2) {
            selectedMeso = selection;
            if (selectedType == 0) {
                cm.askYesNo("您是否要存入#r " + selectedMeso + " #k亿金币。");
            } else if (selectedType == 1) {
                cm.askYesNo("您是否要取出#r " + selectedMeso + " #k亿金币。");
            }
        } else if (status == 3) {
            if (selectedType == 0) {
