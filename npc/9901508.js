...
function startChat(type, selection)
{
    if (cm.getMapId() == 180000001) {
        if (chat == 0)
            cm.askYesNo("Do you want to get out?");
        else if (chat == 1) {
            cm.warp(100000000);
            cm.dispose();
        }
        ...
        if (chat == 0)
            cm.askMenu("Hello, I can bring you to a map where you can drop untradable items.\r\n#L0#I want to go there#l\r\n#L1#I might come back later#l");
        else if (chat == 1) {
            switch (selection) {
                case 1:
                    ...
                    break;
                case 0:
                    cm.askText("Please enter your partner's name: \r\n");
            }
        } else if (chat == 2) {
