...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        cm.askMenu("What do you want to change?\r\n#b#L0#Auto HP#l\r\n#L1#Auto MP#l");
    } else if (status == 1) {
        firstSel = selection;
        if (selection == 0) {
            if (!cm.getPlayer().getStat().hasHP) {
                ...
                cm.askNumber("Enter what % HP you want the Auto Potion to activate. (1-100)", 50, 1, 100);
            }
        } else {
            if (!cm.getPlayer().getStat().hasMP) {
                ...
                cm.askNumber("Enter what % MP you want the Auto Potion to activate. (1-100)", 50, 1, 100);
            }
        }
        ...
        if (firstSel == 0) {
            if (selection >= 1 && selection <= 100) {
                ...
                if (selStr.length <= 0) {
                    ...
                    cm.askMenu("The Percent has been set. Please choose what item to set as the Potion:\r\n" + selStr);
                }
            } else {
