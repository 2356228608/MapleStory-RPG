...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (cm.getMapId() == 100000000) {
            cm.askYesNo("I can take you to the Amoria Village. Are you ready to go?");
        } else if (cm.getMapId() == 680000500 || cm.getMapId() == 680000501 || cm.getMapId() == 680000502) {
            cm.setQuestRecord(cm.getPlayer(), 160002, "0");
            cm.warp(680000000, 0);
            cm.dispose();
        } else {
            cm.askYesNo("I can take you back to your original location. Are you ready to go?");
        }
    } else if (status == 1) {
        cm.askMenu("I hope you had a great time! See you around!");
    } else if (status == 2) {
        if (cm.getMapId() == 100000000) {
            cm.warp(680000000, 0);
        } else {
