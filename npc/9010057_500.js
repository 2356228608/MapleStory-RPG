...
function deleteItemBySlot(selection) {
    if (status == 1) {
        text = head + "#e- 请选择要回收的道具类型 -(注意:背包没东西别乱点，造成物品数据丢失)#n\r\n#b";
        text += "\t#L1#装备栏#l\r\n";
        text += "\t#L2#消耗栏#l\r\n";
        text += "\t#L4#其它栏#l\r\n";
        text += "\t#L3#设置栏#l\r\n";
        ...
        text = head + "#e确定要回收#r#v" + item.getItemId() + "##z" + item.getItemId() + "# " + deleteQuantity + "个 #k吗？";
        cm.sendNextPrev(text);
    } else if (status == 4) {
        cm.removeSlot(inventoryType, deleteSlot, deleteQuantity);
        cm.sendOk(head + "回收成功，祝你游戏愉快~");
        status = 0;
    }
}