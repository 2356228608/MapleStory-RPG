...
function CheckPlayerPosition(answers) {//通过答案查看玩家的所站的位置是否正确
    ...
    //-369 0  -277 -87  -171 -272 -89 -368 8
    //cm.spouseMessage(0x24, "[答案为:] "+answers+" 当前玩家坐标: "+Xpos);
    if (Xpos >= -562 && Xpos <= 150) {//X部分
        if (answers) {
            if (em.getProperty("OXEventState") > 5) {
                cm.warp(910000000, 0);//
                cm.sendOk("嗯……。回答错误，罚你出去！");
                cm.addPopupSay(1540205, 1000, "真遗憾呢……！下次再接再厉吧！！");//显示10秒
            } else {
                cm.fieldEffect_ScreenMsg(false, "quest/party/wrong_kor");
                cm.fieldEffect_PlayFieldSound("Party1/Failed");
            }
            cm.dispose();
        } else {
            cm.fieldEffect_ScreenMsg(false, "quest/party/clear");
        ...
        if (em.getProperty("OXEventState") > 5) {
            ...
            cm.addPopupSay(1540205, 1000, "真遗憾呢……！下次再接再厉吧！！");//显示10
        } else {
            cm.fieldEffect_ScreenMsg(false, "quest/party/wrong_kor");
            cm.fieldEffect_PlayFieldSound("Party1/Failed");
            //cm.sendOk("嗯……。这是一个对或错的问题，你站中间是几个意思？");
