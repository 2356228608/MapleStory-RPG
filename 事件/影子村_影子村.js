...
var reviveCount = 5;
var mapID = 307090020;
var mapIds = [307090020];
var mapHall = 307000100;
var eventName = "影子村_影子村";
...
function setup(eim, leaderid) {
	...
	for (var i = 0; i < mapIds.length; i++) {
        ...
        modified.setOHp(30000000);//20E
        modified.setOMp(mob.getMobMaxMp() * 50);
	modified.setOExp(200000);
        mob.getStats().setPhysicalAttack(2000);//物理伤害
        mob.getStats().setMagicAttack(2000);//魔攻伤害
        mob.getStats().setPDRate(10);
        mob.getStats().setMDRate(10);
        mob.getStats().setLevel(180);
	mob.getStats().setAcc(500000);
        ...
        mapForMob.spawnMonsterOnGroundBelow(mob, new java.awt.Point(-1450, -6));
	...
	mob.getStats().setAcc(500000);
        ...
        eim.registerMonster(mob);
        var mapForMob = eim.getMapInstance(mapID);
        mapForMob.spawnMonsterOnGroundBelow(mob, new java.awt.Point(-839, -6));
...
function monsterValue(eim, mobId) {
	...
	case 1:
		...
		eim.getPlayers().forEach(function (player) {
			...
			player.dropMessage(-1, "消灭数量："+(kill)+"/2");
		});
		//击杀达标时
		if (kill >= kilReq) {
			em.setProperty("stage" + state, "clear");
			eim.getPlayers().forEach(function (player) {
				player.openNpc(0, "32988_战斗结束");
			});
