...
function setup(level, leaderid) {
    ...
    em.setProperty("leader", "true");
    var eim = em.newInstance("zhianmojin_HARD");
	var map = eim.setInstanceMap(450009500);
        map.resetFully();
...
function playerEntry(eim, player) {
	...
	player.dropMessage(6, "["+player.getName()+"]玩家已开始挑战[至暗魔晶-进阶],祝福拿到战利品归来。");
}
...
function changedMap(eim, player, mapid) {
    ...
    if(mapid == 450009500){
		...
		//player.dropMessage(6, "[至暗魔晶-进阶] 已退出挑战,副本空闲,强者可以去挑战。");
		eim.worldSpouseMessage(0x24, "[至暗魔晶-进阶] 已退出挑战,副本空闲,强者可以去挑战。");
		eim.unregisterPlayer(player);
...
function playerDisconnected(eim, player) {
    eim.disposeIfPlayerBelow(100, 450009500);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
    return 0;
}
...
function monsterValue(eim, mobid) {
    switch (mobid) {
		case 8644650:
			var map = eim.getMapInstance(450009500);
