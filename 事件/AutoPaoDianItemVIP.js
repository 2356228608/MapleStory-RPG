...
function scheduleNew() {
    ...
    while (nextTime <= java.lang.System.currentTimeMillis()) {
        nextTime += em.AutoPaodianItemtimeVIP();
    }
...
function start() {
    ...
    while (allPlayers.hasNext()) {//循环每一个玩家
        var player = allPlayers.next();
			if (player.getMapId() == em.AutoPaodianmapVIP() && player.haveItem(em.AutoPaodianItemVIP()) && em.AutoPaodianItemOnVIP()) {
				var mode = em.AutoPaodianItemModeVIP();
