...
function setup() {
    var eim = em.newInstance("MV");
    var map = eim.setInstanceMap(980010101);
    //map.resetFully();
    //map.killAllMonsters(true);
    map.respawn(false);
    eim.startEventTimer(1000 * 60 * 5);//缩短为5分钟
    em.setProperty("FriendlyTips", "0");
...
function EndThisBattle(eim) {
    ...
    var resultAll = Array();
    for (var i = 0; i < eim.getPlayerCount(); i++) {
        result.push(eim.getPlayers().get(i).getName())
        result.push(eim.getKillCount(eim.getPlayers().get(i)))
    ...
    for (var i = 0; i < eim.getPlayerCount(); i++) {
        ...
        //em.broadcastServerMsg(5120026, "[阿里安特竞技场] 今天竞技场现在已经结束。恭喜玩家：" + resultAll[i][0] + "获得第一名。", true);
        //em.broadcastServerMsg(6, "[阿里安特竞技场] 今天竞技场现在已经结束。恭喜玩家：" + resultAll[i][0] + "获得第一名。", false);
    }
