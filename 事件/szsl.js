...
function monsterSpawn(eim) {
    var map = eim.getMapInstance(940021000);
    var pfB = em.fieldEffect_ScreenMsg("Visitor/RankB");
    var pfA = em.fieldEffect_ScreenMsg("Visitor/RankA");
    var pfF = em.fieldEffect_ScreenMsg("Visitor/RankF");
    var pfS = em.fieldEffect_ScreenMsg("Visitor/RankS");
    var packetsnd = em.fieldEffect_PlayFieldSound("rootabyss/firework");
    if (em.getProperty("state").equals("1") == true) {
        ...
        for (var i = 0; i < 5; i++) {
            ...
            map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(2076, 29)); //刷出这个怪物
        }
        for (var i = 0; i < 5; i++) {
            ...
            mob.setHp(hprand);
            eim.registerMonster(mob);
            map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(2507, 29)); //刷出这个怪物
        }
        for (var i = 0; i < 5; i++) {
            ...
            mob.setHp(hprand);
            eim.registerMonster(mob);
            map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(2967, 29)); //刷出这个怪物
        }
        for (var i = 0; i < 5; i++) {
            ...
            mob.setHp(hprand);
            eim.registerMonster(mob);
            map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(3407, 29)); //刷出这个怪物
        }
...
function beginQuest(eim) {
    em.getMapFactory().getMap(940021000).startMapEffect("[小试牛刀]10秒后第一波黑暗骑士将到达，干掉它们！！", 5120092);
    //em.getMapFactory().getMap(940021000).startSimpleMapEffect("10秒后第一波次元入侵者将到达，阻止他们！！",5120092);
