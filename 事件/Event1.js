...
function startEvent() {
    if (em.getProperty("start") == "1") {//已经可以让后面的玩家进来了。
        if (eim.getMapFactory().getMap(mapid).getCharactersSize() == 0) {
            ...
            em.broadcastServerMsg("[OX宾果活动] OX宾果活动已经开始了，现在大约有1分钟的报名时间，请速度到拍卖报到！");
            em.setProperty("start", "2");//等待状态
            setupTaskEvent = em.schedule("WatingStatus", 1000 * 60 * 1, eim);//3分钟后检查问题
        }
...
function WatingStatus(eim) {
    if (em.getProperty("start") == "2") {//等待状态
        ...
        if (eim.getMapFactory().getMap(mapid).getCharactersThreadsafe() != 0) {//如果开始了的话
            ...
            for (var i = 0; i < eim.getMapFactory().getMap(mapid).getCharactersSize(); i++) {
                if (eim.getMapFactory().getMap(mapid).getCharactersSize() != 0) {
                    eim.getMapFactory().getMap(mapid).getCharactersThreadsafe().get(i).getClient().getSession().write(Packages.tools.packet.CField.getClock(10));//10秒
...
function QuetionStart(eim) {//问题提出部分
    if (asked.length != 20) {
        ...
        for (var i = 0; i < eim.getMapFactory().getMap(mapid).getCharactersSize(); i++) {
            if (eim.getMapFactory().getMap(mapid).getCharactersSize() != 0) {
                eim.getMapFactory().getMap(mapid).getCharactersThreadsafe().get(i).getClient().getSession().write(Packages.tools.packet.CField.getClock(15));//15秒
        ...
        for (var i = 0; i < eim.getMapFactory().getMap(mapid).getCharactersSize(); i++) {
            if (eim.getMapFactory().getMap(mapid).getCharactersSize() != 0) {
                eim.getMapFactory().getMap(mapid).getCharactersThreadsafe().get(i).openNpc(9000277, 3);//余下人员奖励NPC,这个NPC要直接把玩家送出地图
            }
        }
...
function AfterQuestion(eim) {//问题检查部分
    ...
    for (var i = 0; i < eim.getMapFactory().getMap(mapid).getCharactersSize(); i++) {
        if (eim.getMapFactory().getMap(mapid).getCharactersSize() != 0) {
            ...
            eim.getMapFactory().getMap(mapid).getCharactersThreadsafe().get(i).getClient().getSession().write(Packages.tools.packet.CField.getClock(5));//5
        }//避免报错
    }
