...
var godman = [10000, 10100, 10110, 10111, 10112];
    ...
    function sentWeapon() {
            ...
            var toDrop = ii.randomizeStats(ii.getEquipById(1372222)).copy();            //生成一个Equip类
            // toDrop.setStr(200);                                                         //装备力量
            // toDrop.setDex(200);                                                         //装备敏捷
            // toDrop.setInt(200);                                                         //装备智力
            // toDrop.setLuk(200);                                                         //装备运气
			// toDrop.setPAD(80); //物理攻击
			// toDrop.setMAD(80); //魔法攻击
			// toDrop.setSpeed(10); //移动速度
			// toDrop.setJump(10); //跳跃
			// toDrop.setMaxHP(6000);
			// toDrop.setMaxMP(6000);//mp
			// toDrop.setBDR(5);//BOSS伤害//最高255
			// toDrop.setIMDR(5);//无视防御//最高255
			// toDrop.setDamR(5); //总伤害//最高255
			// toDrop.setStatR(5); //全属性//最高255
			// toDrop.setPDD(20);
			// toDrop.setMDD(20);//防御力
			//toDrop.setCHUC(15);
			toDrop.setRUC(0);
			//潜能
			...
			toDrop.setOption(potList[Math.floor(Math.random()*potList.length)][0],3,false);
			//附加潜能
			// toDrop.setOption(potList[Math.floor(Math.random()*potList.length)][0],1,true);
			// toDrop.setOption(potList[Math.floor(Math.random()*potList.length)][0],2,true);
			// toDrop.setOption(potList[Math.floor(Math.random()*potList.length)][0],3,true);
			// toDrop.resetPotential(6,true,true);
