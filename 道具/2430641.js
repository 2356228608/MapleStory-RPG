	...
	if (status == 0) {
		...
		if (currentMapId == mapid) {
			...
			if (randomNum <= 10) {
				...
				im.worldSpouseMessage(0x24, "『寻宝奇遇』 : <" + im.getChar().getName() + "> 挖宝不慎，在"+im.getPlayer().getClient().getChannel()+"线<"+mapName+">召唤出了御龙魔，各位侠士速速去击杀。");
				im.dispose();
			} else {
				...
				//im.worldSpouseMessage(0x24, "『寻宝奇遇』挖到了#b#v"+giftId+"##t"+giftId+"##k"+giftQuantity+"个。");
				var gachaponItem = im.gainGachaponItem(giftId, 1, "『寻宝奇遇』", 3, false);
				im.dispose();
