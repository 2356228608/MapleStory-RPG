...
function start(mode, type, selection) {
	...
	if (status <= i++) {
		...
		qm.sendNextNoESC("我向你的勇气表示致意。请你先去消灭封印守护者，解开世界树的封印。然后请你把世界树安全地带到圣地。", 1101002);
	} else if (status === i++) {
		qm.sendNextNoESC("冒险岛联盟已经公告了营救世界树的行动。行动快的人也许已经到达鲁塔比斯了。请你和他们一起，救出世界树。", 1101002);
	} else if (status === i++) {
		qm.sendNextNoESC("以后，你可以通过次元之境，或冒险岛向导，移动到鲁塔比斯。", 1101002);
	} else if (status === i++) {
		// 收尾		
		qm.forceStartQuest();
		qm.dispose();
