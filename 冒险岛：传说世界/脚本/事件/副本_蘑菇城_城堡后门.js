 ...
 * by Jessefjxm
 */
var mapIds = [106031200];
var mapHall = 106031210;
...
function playerEntry(eim, player) {
	...
	player.dropMessage(-1, "守护城门！");
	scheduleNew("attack", 10);
}
...
function attack() {
	if (em.getTimeLeft() < 5 * 1000) {
		return;
	}
	var eim = em.getInstance("副本_蘑菇城_城堡后门");
	var mobId = 3300109;
	var map = em.getMapFactoryMap(mapIds[0]);
	var mob = em.getMonster(mobId);
	eim.registerMonster(mob);
	for (var i = 0; i < 10; i++) {
		map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(1000 + i * 50, 200));
	}
	scheduleNew("attack", 10);
}
