 ...
 * by Jessefjxm
 */
var mapIds = [211060201];
var mapHall = 211060200;
var emName = "副本_狮子王城_第一座塔";
...
function setup(eim, leaderid) {
	...
	for (var i = 0; i < 5; i++) {
		var mob = em.getMonster(8840002);
		eim.registerMonster(mob);
		map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(randomNum(-150, 1200), -170));
	}
	return eim;
}
...
function monsterValue(eim, mobId) {
	...
	if (kill >= kilReq) {
		eim.getPlayers().forEach(function (player) {
			...
			player.dropMessage(-1, "狮子王之城的第一个封印解开了。");
			player.updateInfoQuest(3139, "clear");
		});
	}
	return 1;
}
