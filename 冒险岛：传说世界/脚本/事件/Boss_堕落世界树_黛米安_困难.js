...
function setup(level, leaderid) {
	var eim = em.newInstance("Boss_堕落世界树_黛米安_困难");
...
function monsterValue(eim, mobid) { 
	...
	if (mobid == 8880100) { //戴米安1
		...
		eim.getPlayers().forEach(function (player) {
			player.updateAchievement(819, 0, "mob_kill", 1, 1);
			player.updateAchievement(820, 0, "mob_kill", 1, 10);
			player.updateAchievement(821, 0, "mob_kill", 1, 100);
			player.updateAchievement(822, 0, "mob_kill", 1, 100);
		});
		//  组队通关
		if (teamMembers == 4) {
			player.gainAchievement(823, 0, "mob_kill=1");
		} else if (teamMembers == 5) {
			player.gainAchievement(823, 1, "mob_kill=1");
		} else if (teamMembers == 6) {
			player.gainAchievement(823, 2, "mob_kill=1");
		}
		// 限时通关
		if (1000 * 60 * 40 - eim.getTimeLeft() < 5 * 60 * 1000) {
			player.finishAchievement(824);
		}
	}
