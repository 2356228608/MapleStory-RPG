...
function setup(level, leaderid) {
	var eim = em.newInstance("Boss_堕落世界树_黛米安_普通");
...
function monsterValue(eim, mobid) {
	...
	if (mobid == 8880100) { //戴米安1
		...
		eim.getPlayers().forEach(function (player) {
			player.updateAchievement(811, 0, "mob_kill", 1, 1);
			player.updateAchievement(812, 0, "mob_kill", 1, 10);
			player.updateAchievement(813, 0, "mob_kill", 1, 100);
			player.updateAchievement(814, 0, "mob_kill", 1, 100);
		});
		//  组队通关
		if (teamMembers == 1) {
			player.finishAchievement(815);
		} else if (teamMembers == 4) {
			player.gainAchievement(816, 0, "mob_kill=1");
		} else if (teamMembers == 5) {
			player.gainAchievement(816, 1, "mob_kill=1");
		} else if (teamMembers == 6) {
			player.gainAchievement(816, 2, "mob_kill=1");
		}
	}
	return 1;
}
