 ...
 * by Jessefjxm
 */
var mapIds = [921140000,921140001];
var mapHall = 211061001;
var emName = "副本_狮子王城_秘密走廊";
...
function setup(eim, leaderid) {
	...
	for (var i = 0; i < 20; i++) {
		var mob = em.getMonster(9300834);
		eim.registerMonster(mob);
		map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(randomNum(-2000, 1400), -140));
	}
	var mob = em.getMonster(9300295);
	eim.registerMonster(mob);
	map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(1576, -140));
	return eim;
}
...
function playerEntry(eim, player) {
	...
	player.changeMap(map, map.getPortal(0));
			player.dropMessage(-1, "清空前往接待室的道路，带着伊帕娅前进吧。");
...
function monsterValue(eim, mobId) {
	...
	if (kill >= kilReq) {
		eim.getPlayers().forEach(function (player) {
			...
			player.dropMessage(-1, "前往接待室的道路已经清空了。可以带着伊帕娅前进了。");
			player.updateInfoQuest(3178, "clear");
		});
	}
	return 1;
}
