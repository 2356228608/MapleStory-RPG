 ...
 * by Jessefjxm
 */
var mapIds = [211060601];
var mapHall = 211060600;
var emName = "副本_狮子王城_第三座塔";
...
function setup(eim, leaderid) {
	...
	for (var i = 0; i < 5; i++) {
		var mob = em.getMonster(8210007);
		eim.registerMonster(mob);
		map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(randomNum(1200, 1500), -170));
	}
	return eim;
}
...
function monsterValue(eim, mobId) {
	...
	if (kill >= kilReq) {
		eim.getPlayers().forEach(function (player) {
			...
			player.dropMessage(-1, "狮子王之城的第三个封印解开了。");
			player.updateInfoQuest(3141, "clear");
		});
	}
	return 1;
}
