...
function setup(eim, leaderid) {
	...
	em.setProperty("leader", "true");
	var eim = em.newInstance("副本_起源之塔");
...
function monsterValue(eim, mobId) {
	...
	case 13:
		...
		if (kill == kilReq * 4) {
			...
			em.getMapFactoryMap(mapid).getWeatherEffectNotice("咳……幸好全部消灭了。你做得不错。让我们通过右侧边缘的传送出口前往下一层吧。", 147, 30000, 1);
	...
	case 14:
		...
		if (kill >= kilReq) {
			...
			em.getMapFactoryMap(mapid+2).getWeatherEffectNotice("做得很好！赶快从右侧的出口下去吧。", 147, 30000, 1);
		}
		break;
	case 16:
		var kilReq = 200;
	...
	case 37:
		...
		if (kill[0] >= kilReq && kill[1] >= kilReq && kill[2] >= kilReq && kill[3] >= kilReq) {
			...
			em.getMapFactoryMap(mapid).getWeatherEffectNotice("你已经将所有的怪物各消灭了100个。让我们前往下一层吧。", 147, 30000, 1);
	...
	case 42:
		...
		if (kill[0] >= kilReq && kill[1] >= kilReq && kill[2] >= kilReq && kill[3] >= kilReq) {
			...
			em.getMapFactoryMap(mapid).getWeatherEffectNotice("你已经将所有的怪物各消灭了30个。请前往下一层。", 147, 30000, 1);
			eim.getPlayers().forEach(function (player) {
	...
	case 47:
		if (mobId == 9309128) {
			...
			em.getMapFactoryMap(mapid).getWeatherEffectNotice("咳。现在我的声音才能传到你那儿。我已经把门打开了，请前往下一层吧。", 147, 30000, 1);
		}
		break;
	case 50:
		if (mobId == 9309207) {
...
function stage3_CountItem() {
	...
	em.getPlayersInMap(mapid).forEach(function (player) {
		...
		if (owns < reqNum) {
			...
			em.getMapFactoryMap(mapid).getWeatherEffectNotice("哇！你真的收集到1000个啦？现在我们可以前往下一层了。", 147, 30000, 1);
...
function stage18_CountItem() {
	...
	em.getPlayersInMap(mapid).forEach(function (player) {
		...
		if (owns < reqNum) {
			...
			em.getMapFactoryMap(mapid).getWeatherEffectNotice("终于收集够椰子果了。现在我们可以前往下一层了。", 147, 30000, 1);
...
function stage34_CountItem() {
	...
	em.getPlayersInMap(mapid).forEach(function (player) {
		...
		if (owns < reqNum) {
			...
			em.getMapFactoryMap(mapid).getWeatherEffectNotice("你已经将10个紫色皮革全部搜集到了！让我们前往下一层吧！", 147, 30000, 1);
