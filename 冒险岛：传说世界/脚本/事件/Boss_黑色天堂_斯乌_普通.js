...
function setup(level, partyid) {
	...
	em.setProperty("leader", "true");
	var eim = em.newInstance("Boss_黑色天堂_斯乌_普通");
...
function monsterValue(eim, mobId) {
	...
	eim.getPlayers().forEach(function (player) {
		if (player.getMapId() != 350060600)
			return 0;
		player.updateAchievement(797, 0, "mob_kill", 1, 1);
		player.updateAchievement(798, 0, "mob_kill", 1, 10);
		player.updateAchievement(799, 0, "mob_kill", 1, 100);
		player.updateAchievement(800, 0, "mob_kill", 1, 100);
		//  组队通关
		if (teamMembers == 1) {
			player.finishAchievement(801);
		} else if (teamMembers == 4) {
			player.gainAchievement(802, 0, "mob_kill=1");
		} else if (teamMembers == 5) {
			player.gainAchievement(802, 1, "mob_kill=1");
		} else if (teamMembers == 6) {
			player.gainAchievement(802, 2, "mob_kill=1");
		}
	});
	return 1;
}
