...
function end(mode, type, selection) {
	...
	if (status <= i++) {
		...
		qm.sendNextNoESC("新地区的探索结束了吗？一直联系不上你，我还担心会不会出什么事了呢。", 1101002);
	} else if (status === i++) {
		qm.sendNextSNoESC("这段时间发生了很多事情。");
	} else if (status === i++) {
		qm.sendNextSNoESC("（向南哈特讲述在鲁塔比斯发生的事情。）");
	} else if (status === i++) {
		qm.sendNextNoESC("……真让人吃惊。如果这一切是真的，那就是非常大的事件。很久以前消失了的世界树又重新出现，是件好事。但是你说有一群来历不明的人正在觊觎世界树的力量，这是个非常严重的问题。", 1101002);
	} else if (status === i++) {
