...
function start(mode, type, selection) {
	...
	if (status <= i++) {
		...
		qm.sendNextSNoESC("难……难道是进了那座房子……？");
	} else if (status === i++) {
		qm.inGameDirectionEvent_MoveAction(2);
		qm.inGameDirectionEvent_AskAnswerTime(1500);
	} else if (status === i++) {
		qm.inGameDirectionEvent_MoveAction(0);
		qm.sendNextSNoESC("你好……");
	} else if (status === i++) {
		qm.sendNextSNoESC("有人吗……？");
	} else if (status === i++) {
		qm.sendNextSNoESC("是……是鬼魂吗……？");
	} else if (status === i++) {
		qm.sendNextSNoESC("呃……但是如果回到刚才那片森林……就又会迷路……被刺戳……被鸟啄……");
	} else if (status === i++) {
		qm.sendNextSNoESC("怎么办呢……");
	} else if (status === i++) {
		qm.curNodeEventEnd(true);
