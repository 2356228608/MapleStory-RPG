...
function start(mode, type, selection) {
	...
	if (info == null || !info.equals("start")) {
		startOff(mode, type, selection);
	} else {
		startOn(mode, type, selection);
	}
}

function startOff(mode, type, selection) {
	if (status == 0 && mode == 0) {
		qm.dispose();
		return;
	}
...
function startOn(mode, type, selection) {
	if (status == 0 && mode == 0) {
		qm.dispose();
		return;
	}
	...
	if (status <= i++) {
		...
		qm.askMenu("做好开始#e<大冒险钻头>#n的准备了吗？\r\n\r\n#L0#进入#e<大冒险钻头>#n实战模式#l\r\n#L1#进入#e<大冒险钻头>#n练习模式#l\r\n#L2#听一下#e<大冒险钻头>#n的说明#l", 9062147);
	} else if (status > 0) {
		if (selectionLog[1] == 0) {
...
function start2(mode, type, selection) {
	if (status === i++) {
		qm.sendNextPrev("#b#e<大冒险钻头>#k#n是在1分钟内消灭掉尽可能多的#e稻草人怪物#n的任务。", 9062147);
	} else if (status === i++) {
		qm.sendNextPrev("在总共10个踏板上配置有4种不同的稻草人怪物。", 9062147);
