function action(mode, type, selection) {
	...
	case 1:
		...
		if (rand == 1) {
			ms.getMap().startSimpleMapEffect("终于，来到这里……面对真正的对决！来吧！", 5120063);
			spawnMob(em, eim, map, level, Math.pow(level, 2.5), 9410191, 50, 240);
		} else if (rand == 2) {
			ms.getMap().startSimpleMapEffect("真是……不懂得放弃的人啊，告诉你吧。", 5120064);
			spawnMob(em, eim, map, level, Math.pow(level, 2.5), 9410192, 50, 240);
		} else if (rand == 3) {
			ms.getMap().startSimpleMapEffect("终于，来到这里……面对真正的对决！来吧！", 5120065);
			spawnMob(em, eim, map, level, Math.pow(level, 2.5), 9410193, 50, 240);
		} else {
			ms.getMap().startSimpleMapEffect("终于，来到这里……面对真正的对决！来吧！", 5120066);
	...
	case 3:
		...
		if (rand == 1) {
			ms.getMap().startSimpleMapEffect("很感谢你的心意，但是可以训练多少……", 5120069);
			spawnMob(em, eim, map, level, level * level, 9410178, 50, 240);
		} else if (rand == 1) {
			ms.getMap().startSimpleMapEffect("竟敢说要教训我？那就来试试看！呵呵呵", 5120068);
			spawnMob(em, eim, map, level, level * level, 9410177, 50, 240);
		} else {
			ms.getMap().startSimpleMapEffect("嘿嘿~看你怎么训练我。", 5120073);
	...
	case 4:
		...
		ms.getMap().startSimpleMapEffect("刚刚那个像是一直在等谁的小孩就是你吗？", 5120120);
		break;
	case 5:
		// 744000005 - 枫之高校 - 保健室
		// 算你好运。居然躲掉接种预防针？下次不会放过你。
		em.scheduleAtTimestamp("passRoom5", java.lang.System.currentTimeMillis() + 60 * 1000);
		ms.getMap().startSimpleMapEffect("讨厌打针的话，就得避开保健室老师顶住1分钟。", 5120120);
	...
	case 6:
		...
		ms.getMap().startSimpleMapEffect("请帮帮我~僵尸竟然这么多……", 5120076);
	...
	case 7:
		...
		// 让我看看你从库梅老师那里学到了什么吧
		if (rand == 1) {
			ms.getMap().startSimpleMapEffect("我会比之前的班主任库梅老师教得更好。", 5120120);
		} else {
			ms.getMap().startSimpleMapEffect("身为导师绝不原谅打架这件事！只能用力量来阻止你！", 5120067);
		}
		spawnMob(em, eim, map, level, level * level, 9410171, 70, 240);
		break;
	case 8:
		// 744000008 - 枫之高校 - 小考考场
		ms.getMap().startSimpleMapEffect("不通过考试的话，谁都不能出去。", 5120120);
		break;
	case 9:
		// 744000009 - 枫之高校 - 家政教室
		ms.getMap().startSimpleMapEffect("那些家伙把人参带走了。赶快帮一下忙！", 5120020);
	...
	case 10:
		...
		ms.getMap().startSimpleMapEffect("这次大家要一起合作。四大天王中任一位被击倒的话，就算失败啰！", 5120064);
	...
	case 11:
		...
		// 授课已经结束了。接受物理老师的考核吧
		ms.getMap().startSimpleMapEffect("开始上物理课。如果打的力太大会更痛，打的力太小则只会造成小伤害。", 5120071);
		spawnMob(em, eim, map, level, level * level, 9410180, 50, 240);
		break;
	case 12:
		// 744000012 - 枫之高校 - 图书室
		// 授课已经结束了。接受历史老师的考核吧
		ms.getMap().startSimpleMapEffect("开始上历史课，以前的英雄们是这样攻击黑魔法师……", 5120070);
		spawnMob(em, eim, map, level, level * level, 9410181, 50, 240);
		break;
	case 13:
		// 744000013 - 枫之高校 - 空教室
		// 呼~太手下留情了，下次一定要把你带走
		ms.getMap().startSimpleMapEffect("我是来招募新生的……今天要来把你抓走！呼呼呼~", 5120072);
		spawnMob(em, eim, map, level, level * level, 9410182, 50, 240);
		break;
	case 14:
		// 744000014 - 枫之高校 - 柔道部
		// 真是的……不允许有社员比社长还强。不准入社！
		ms.getMap().startSimpleMapEffect("进到柔道部……一点都不用害怕。就是入社测试！！", 5120075);
		spawnMob(em, eim, map, level, level * level, 9410183, 50, 240);
		break;
	case 15:
		// 744000015 - 枫之高校 - 乐队部
		// 不懂艺术……音乐部不需要你……
		ms.getMap().startSimpleMapEffect("我想到了不得了的乐曲……我再弹给你听~呵呵呵", 5120074);
		spawnMob(em, eim, map, level, level * level, 9410184, 50, 240);
		break;
	case 16:
		// 744000016 - 枫之高校 - 伊昆领域
		ms.getMap().startSimpleMapEffect("既然你已经赢了最厉害的问题学生，那就接受我的考核吧。", 5120120);
