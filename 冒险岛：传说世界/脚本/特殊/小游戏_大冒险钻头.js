...
var open = true;
//配置文件名称
var PQname = ["小游戏_大冒险钻头_练习", "小游戏_大冒险钻头"];
//记录次数名称
var PQLog = ["小游戏_大冒险钻头_练习", "小游戏_大冒险钻头"];
//开始的地图
...
function startEvent() {
	var em = cm.getEventManager(PQname[selectionLog[1]]);
	if (em == null || open == false) {
		cm.sendOk("配置文件不存在,请联系管理员。", 9062147);
	} else if (cm.getPQLog(PQ[selectionLog[1]]) >= maxenter[selectionLog[1]]) {
		cm.sendOk("你今天挑战次数已经用完了,请明天在来吧!");
	} else {
		var prop = em.getProperty("state");
		if (prop == null || prop.equals("0")) {
			cm.setPQLog(PQ[selectionLog[1]]);
