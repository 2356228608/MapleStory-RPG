...
function action(mode, type, selection) {
    ...
    var i = -1;
        if (status <= i++) {
			...
			cm.sendNextSNoESC_Bottom("#b啊，吓死我了！这是什么？");
		} else if (status === i++) {
			...
			switch(selection){
				...
				break;
				case 1:
					cm.sendNextSNoESC_Bottom("等想好了我再看看吧！");
				break;
			}
		} else if (status === i++) {
			cm.curNodeEventEnd(true);//角色站立方向信息
